...
from deepblue.reference import Engine
...
def get_move(fen: str, time_left_ms: int) -> str:
    ...
    try:
        ...
        _moves_played += 1
        if hard_ms > 0.0:
            result = _engine.search(board, soft_ms, hard_ms)
            if result.move is not None and result.move in board.legal_moves:
                ...
                print(
                    ...
                    f"nps {result.nodes / max(result.elapsed_ms, 1.0) * 1000:.0f}",
                    file=sys.stderr,
                )
            elif result.move is not None:
                print(f"deepblue: search returned illegal {result.move}", file=sys.stderr)
    except Exception:
