...
fastsearch183 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.
...
fastsearch183 (2026-09-09) = fastsearch180 + a repetition rule that fires only
at ply 1.

Round 79 was a draw from a position the engine rated +195. At move 58 it
played Qc8 -- the SECOND occurrence, which is not a draw, so nothing stopped
it. Black repeated in turn and the referee claimed the threefold before the
search ever reached a node the existing rule would have rejected. A full
point, from a won game, with no rule broken.

fastsearch173 tried the blunt version: score ONE prior occurrence as
-CONTEMPT at every ply. The penalty fired correctly -- round 79 move 56 drops
from +166 to +20 -- but it still chose a repeating move. The reason is that in
a shuffling endgame the same positions recur all over the tree, so large parts
of the search collapse to +/-CONTEMPT and the engine loses the ability to tell
its options apart. Making everything a draw is not the same as avoiding one.

This restricts the new rule to ply == 1, the nodes that correspond to a move
we are about to PLAY. Repeating on the board is discouraged; the deep search
keeps its normal threefold semantics and its discrimination. -CONTEMPT is a
cap, not a refusal: at +195 the engine walks away from the repetition, and at
-300 it still takes it gratefully.
"""
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if ply > 0 and not suppress_repetition:
        ...
        # See the module docstring: applying this at every ply (fastsearch173)
        # collapses a shuffling endgame's whole tree to +/-CONTEMPT and the
        # engine can no longer rank its options.
