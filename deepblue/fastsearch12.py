"""S1-search12: PVS+RFP plus bounded main/capture/continuation histories.

This experiment matures move ordering without adding any new pruning. Capture
SEE is deliberately NOT used here: Patch 06 proved the SEE implementation but
its first ordering policy was not stronger. This variant first improves the
learned ordering signals that later SEE staging and LMR will depend on.

Baseline search semantics remain those of accepted fastsearch4.

Original baseline description:

accepted PVS baseline plus conservative Reverse Futility Pruning.

This is an independent implementation of the standard PVS idea used across
modern engines (Coda, Stockfish, Reckless, Berserk and others): the first move
at a node is searched with the full alpha-beta window; later moves are probed
with a zero-width window and receive a full re-search only when that probe
shows they can improve alpha.

This variant deliberately adds exactly one new search idea beyond search2: RFP
(static null-move pruning).  The first Deep Blue version is intentionally more
conservative than current Coda/Reckless/Pawnstar because our HCE is much weaker
than their tuned/NNUE evaluations.
...
CUT_TOTAL, CUT_FIRST, CUT_FIRST2, CUT_MOVE_SUM, CUT_TACTICAL, CUT_QUIET = 11, 12, 13, 14, 15, 16
...
# Bounded history tables. Piece codes already include colour, so no additional
# side dimension is needed. The gravity update prevents saturation while
# preserving long-lived evidence across moves of a real game.
MAIN_HISTORY_MAX = 16_384
CAPTURE_HISTORY_MAX = 16_384
CONT_HISTORY_MAX = 16_384
...
def _gravity(old, bonus, limit):
    if bonus > limit:
        bonus = limit
    elif bonus < -limit:
        bonus = -limit
    return old + bonus - (old * abs(bonus)) // limit


@njit(cache=True, nogil=True, inline="always")
def update_main_history(history, piece, to_square, bonus):
    history[piece, to_square] = _gravity(
        int(history[piece, to_square]), bonus, MAIN_HISTORY_MAX
    )


@njit(cache=True, nogil=True, inline="always")
def update_capture_history(capture_history, piece, to_square, captured, bonus):
    capture_history[piece, to_square, captured] = _gravity(
        int(capture_history[piece, to_square, captured]), bonus, CAPTURE_HISTORY_MAX
    )


@njit(cache=True, nogil=True, inline="always")
def update_cont_history(continuation_history, prev_piece, prev_to, piece, to_square, bonus):
    if prev_piece < 0:
        return
    continuation_history[prev_piece, prev_to, piece, to_square] = _gravity(
        int(continuation_history[prev_piece, prev_to, piece, to_square]),
        bonus,
        CONT_HISTORY_MAX,
    )
...
@njit(cache=True, nogil=True)
def order_moves(
    moves, count, scores, mail, tt_move, killers, history, capture_history,
    continuation_history, prev_piece, prev_to, ply
):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # Keep the accepted tactical-before-quiet policy in this experiment;
            # capture history only improves ordering *within* that tactical band.
            learned = int(capture_history[piece, to_square, captured])
            scores[index] = (
                ORDER_CAPTURE + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece] + learned
            )
            ...
            learned = int(history[piece, to_square])
            if prev_piece >= 0:
                learned += int(continuation_history[prev_piece, prev_to, piece, to_square])
            if learned > HISTORY_CEILING:
                learned = HISTORY_CEILING
            elif learned < -HISTORY_CEILING:
                learned = -HISTORY_CEILING
            scores[index] = learned
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            killers, history, capture_history, continuation_history,
            quiet_tried, capture_tried, prev_piece, prev_to,
    ...
    counters[ORDER] += 1
    order_moves(
        pseudo[ply], count, scores[ply], mail, tt_move, killers, history, capture_history,
        continuation_history, prev_piece, prev_to, ply
    )
    ...
    for order_index in range(count):
        ...
        if in_check(bb, occ, st, side):
            ...
            continue
        legal += 1
        moved_piece = int((move >> np.uint32(12)) & np.uint32(15))
        to_square = int((move >> np.uint32(6)) & np.uint32(63))
        captured_piece = int((move >> np.uint32(16)) & np.uint32(15))
        promotion_piece = int((move >> np.uint32(20)) & np.uint32(15))
        is_tactical = captured_piece != NO_PIECE or promotion_piece != NO_PIECE
        if not is_tactical:
            quiet_tried[ply, quiet_count] = move
            quiet_count += 1
        elif captured_piece != NO_PIECE:
            capture_tried[ply, capture_count] = move
            capture_count += 1
        child_prev_piece = promotion_piece if promotion_piece != NO_PIECE else moved_piece
        child_prev_to = to_square
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, depth - 1, -beta, -alpha, ply + 1, child,
                             ...
                             tt_move_arr, killers, history, capture_history, continuation_history,
                             quiet_tried, capture_tried, child_prev_piece, child_prev_to,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
        ...
        if best > alpha:
            alpha = best
        if alpha >= beta:
            counters[CUT_TOTAL] += 1
            counters[CUT_MOVE_SUM] += legal
            if legal == 1:
                counters[CUT_FIRST] += 1
            if legal <= 2:
                counters[CUT_FIRST2] += 1
            if is_tactical:
                counters[CUT_TACTICAL] += 1
            else:
                counters[CUT_QUIET] += 1

            # Update only after a proven beta cutoff. Reward the winner and
            # apply a smaller malus to earlier legal competitors in the same
            # class. This is independent of SEE/pruning and bounded by gravity.
            bonus = 24 * depth * depth + 16 * depth
            malus = -(bonus // 2)
            if not is_tactical:
                if killers[ply, 0] != move:
                    killers[ply, 1] = killers[ply, 0]
                    killers[ply, 0] = move
                update_main_history(history, moved_piece, to_square, bonus)
                update_cont_history(
                    continuation_history, prev_piece, prev_to, moved_piece, to_square, bonus
                )
                for tried_index in range(quiet_count - 1):
                    tried = quiet_tried[ply, tried_index]
                    tried_piece = int((tried >> np.uint32(12)) & np.uint32(15))
                    tried_to = int((tried >> np.uint32(6)) & np.uint32(63))
                    update_main_history(history, tried_piece, tried_to, malus)
                    update_cont_history(
                        continuation_history, prev_piece, prev_to, tried_piece, tried_to, malus
                    )
            elif captured_piece != NO_PIECE:
                capture_bonus = 16 * depth * depth + 8 * depth
                capture_malus = -(capture_bonus // 2)
                update_capture_history(
                    capture_history, moved_piece, to_square, captured_piece, capture_bonus
                )
                for tried_index in range(capture_count - 1):
                    tried = capture_tried[ply, tried_index]
                    tried_piece = int((tried >> np.uint32(12)) & np.uint32(15))
                    tried_to = int((tried >> np.uint32(6)) & np.uint32(63))
                    tried_captured = int((tried >> np.uint32(16)) & np.uint32(15))
                    update_capture_history(
                        capture_history, tried_piece, tried_to, tried_captured, capture_malus
                    )
            break
        if stop[0]:
            break
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
                ...
                tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                killers, history, capture_history, continuation_history, quiet_tried, capture_tried,
                piece_keys, side_key, castle_keys, ep_file_keys,
    ...
    order_moves(
        pseudo[0], count, scores[0], mail, previous_best, killers, history, capture_history,
        continuation_history, -1, 0, 0
    )
...
class FastEngine12:
    ...
    def __init__(self) -> None:
        ...
        self.counters = np.zeros(24, dtype=np.int64)
        ...
        self.capture_history = np.zeros((12, 64, 12), dtype=np.int32)
        self.continuation_history = np.zeros((12, 64, 12, 64), dtype=np.int32)
        self.quiet_tried = np.zeros((MAX_PLY, MAX_MOVES), dtype=np.uint32)
        self.capture_tried = np.zeros((MAX_PLY, MAX_MOVES), dtype=np.uint32)
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                legal = search_root(
                    ...
                    self.killers, self.history, self.capture_history, self.continuation_history,
                    self.quiet_tried, self.capture_tried,
