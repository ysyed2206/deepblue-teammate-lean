"""S1-search17: fastsearch4 + depth-0 check floor and MAX_PLY guard.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if stop[0]:
        return 0
    # Hard safety bound before indexing any ply-sized search buffers.
    if ply >= MAX_PLY - 2:
        counters[EVAL] += 1
        return evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE)
    ...
    # Do not hand an in-check horizon node to capture-only quiescence.  Give it
    # one normal-search ply so evasions receive the full search ordering/TT path.
    side = st[0]
    counters[CHECK] += 1
    checked = in_check(bb, occ, st, side)
    ...
    # ``side`` and ``checked`` were computed before the horizon decision above.
