"""S1-search133: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late move reduction for CAPTURES. Our LMR table already uses Ethereal's
# formula for quiets (0.7844 + ln(d)*ln(m)/2.4696 -- same constants), but the
# guard excludes captures and promotions entirely, so a late-ordered losing
# capture is searched at full depth. Ethereal reduces captures by 3, or by 2
# when the move gives check.
#
# Safer here than it sounds, because fastsearch131 added SEE pruning to the
# main search: captures that lose material by force are already skipped at
# shallow depth, so what remains to be reduced is the tail of ordered-late
# captures that SEE could not condemn outright. Applied only from the same
# move number that quiet LMR uses, and never to the TT move.
LMR_CAPTURE_REDUCTION = 3
LMR_CAPTURE_MIN_MOVES = 4
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            if (
                depth >= LMR_MIN_DEPTH
                and legal >= LMR_CAPTURE_MIN_MOVES
                and not checked
                and move_captured0 != np.uint32(NO_PIECE)
                and move_promotion0 == np.uint32(NO_PIECE)
                and move != tt_move
            ):
                # Late-ordered capture: a flat reduction, as Ethereal does.
