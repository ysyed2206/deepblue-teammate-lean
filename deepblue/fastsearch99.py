"""S1-search99: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Delta pruning (fastsearch99). Quiescence is 59-71% of every node this engine
# searches (counted on real positions from rounds 41-44) and is essentially
# unpruned. Delta pruning is the cheapest possible filter on it: if the current
# stand-pat score, plus the value of the piece being captured, plus a margin
# for the best realistic swing, still cannot reach alpha, then that capture
# cannot rescue the node and its whole subtree is skipped. One array lookup and
# an add -- no exchange evaluation, no make/unmake.
#
# A qsearch delta-pruning attempt is recorded as "neutral" in this project's
# history, but that measurement comes from the 60ms-per-move era, where the
# engine reached depth 4 and quiescence had a completely different shape.
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for index in range(count):
        move = pick_best(pseudo[ply], scores[ply], count, index)
        # Delta pruning. Only for plain captures: a promotion changes material
        # by far more than the captured piece, and this node is not in check
        # (the in-check branch returned earlier), so there is no risk of
        # pruning away the only escape from a mate threat.
        delta_captured = (move >> np.uint32(16)) & np.uint32(15)
        delta_promotion = (move >> np.uint32(20)) & np.uint32(15)
        if (delta_captured != np.uint32(NO_PIECE)
                and delta_promotion == np.uint32(NO_PIECE)):
            if stand_pat + PIECE_VALUE[delta_captured] + DELTA_MARGIN <= alpha:
                continue
        old_castle = st[1]
        old_ep_file = Z.canonical_ep_file(bb, occ, st)
        counters[MAKE] += 1
