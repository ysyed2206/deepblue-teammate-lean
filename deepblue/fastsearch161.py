"""S1-search161: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import (
    game_phase,
    mobility_and_danger_white_relative,
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # Mobility and king danger computed together: both walk every bishop,
    # rook and queen of both sides with the same occupancy, so computing each
    # piece's attacks once removes half the slider work in evaluate().
    # Verified bit-identical to the two separate terms across 150 positions.
    _mob, _danger = mobility_and_danger_white_relative(bb, phase, _TOTAL_PHASE)
    extra += _danger
    extra += _mob
