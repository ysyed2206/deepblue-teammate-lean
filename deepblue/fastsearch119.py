"""S1-search119: fastsearch4 + a transposition table probe/store inside qsearch.
...
# ProbCut (fastsearch119). If a REDUCED-depth search already beats beta by a
# comfortable margin, a full-depth search would almost certainly beat beta
# too, so the node can be cut without paying for it. Like null move and
# razoring this is reduce-then-VERIFY -- the reduced search is real, not an
# assumption from the static eval -- which is the category this engine
# accepts. (It rejects outright discards: LMP scored 5%.)
#
# Its recorded 48.3% rejection is void. That measurement came from the 60ms
# gate where the engine reached depth 4, and ProbCut is gated at depth >= 5,
# so it never executed once in the test that dismissed it.
#
# The margin is deliberately large: a cut here skips a whole subtree on the
# evidence of a shallower search, so it should only fire when the reduced
# result is not close.
PROBCUT_MIN_DEPTH = 5
PROBCUT_MARGIN = 200
PROBCUT_REDUCTION = 4
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # ProbCut. Never at PV or check nodes, and never while already inside a
    # null-move search (can_null is False there), which would stack two
    # speculative reductions on top of each other.
    if (
        not is_pv
        and not checked
        and can_null
