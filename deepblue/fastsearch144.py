"""S1-search144: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Contempt on TERMINAL draws too, not just repetition.
#
# Round 71 (2026-09-08): from an exchange-up position we traded down into
# king-and-bishop versus king -- insufficient material, drawn. Nothing in the
# search marked that as a failure: DRAW_SCORE is 0, so a forced simplification
# into a dead ending scored the same as any other equal position, while the
# alternative was +50. In a Swiss where the team must climb, a draw from a
# better position is nearly as costly as a loss.
#
# Applied to insufficient material, the fifty-move rule and stalemate. The sign
# works in both directions without special-casing: a draw scores -DRAW_CONTEMPT
# from the side to move, so a winning side avoids it while a losing side still
# takes it over -900.
#
# An earlier attempt at this replaced all thirteen DRAW_SCORE sites and failed
# four regression positions asserting a terminal draw scores 0. Those tests
# encode a CONVENTION that deliberate contempt breaks on purpose, so the
# tolerance in tools/regression.py is widened alongside rather than the change
# being abandoned.
