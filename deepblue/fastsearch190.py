...
fastsearch190 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.
...
fastsearch190 (2026-09-09) = fastsearch180 + a SCALE FACTOR by the stronger
side's pawn count.

An advantage you cannot promote is not an advantage. Every strong engine damps
its evaluation toward zero as material approaches configurations that cannot be
won; this engine has never had such a term, and it has cost real games:

  round 88  Rook and bishop against rook, no pawns -- a theoretical draw. Our
            evaluation scored it +410 AT DEPTH 22, and held that for sixty
            moves while the fifty-move counter ran out. Not a search failure:
            33x the thinking time moved it by 2cp.
  round 86  Drawn Q+3P vs Q+3P scored +83, built from doubled pawns and king
            placement -- both real, neither convertible with queens on.
  round 79  Drew a position rated +195 the same way.

The multipliers are FITTED, not chosen: coordinate descent on 120k quiet
positions carrying deep Stockfish scores, 80/20 train/validation split, with
the 6+ pawn bucket PINNED at 1.0 so the fit cannot absorb a global rescale of
our evaluation (letting it float drove every bucket to ~0.38, which is just
re-fitting Texel's K and changes no move ordering at all).

    stronger side's pawns :   0     1     2     3     4     5     6
    fitted multiplier     : 0.14  0.14  0.48  0.67  0.79  0.62  1.00
    shipped below         : 0.14  0.14  0.48  0.67  0.79  0.90  1.00

    validation MSE 0.032198 -> 0.029736   (-0.002461)

For scale, every other evaluation result measured today: refitting all eight
term scales -0.001588, attacked-square king danger -0.000104, doubling
KING_DANGER_SCALE -0.000099. This one is larger than all of them together.

The 5-pawn bucket is the one deliberate departure from the fit. 0.62 breaks an
otherwise monotone ramp and there is no reason a five-pawn advantage should be
less convertible than a four-pawn one; an unexplained non-monotonicity in a
fitted table is usually the fit absorbing something else, so it is smoothed to
0.90 rather than shipped as measured.

RISK. This changes evaluation MAGNITUDE substantially in the endgame, and
reverse futility pruning, razoring and the delta cutoffs all compare against
absolute centipawn margins. A quieter evaluation makes those margins relatively
larger, so the search may prune differently in exactly the positions this term
targets. That interaction is the reason this needs a match and cannot be
shipped on the fit alone.
"""
...
from deepblue.eval_terms import WP, BP, popcount

# Indexed by the stronger side's pawn count; see the module docstring.
PAWN_SCALE_FACTOR = np.array([14, 14, 48, 67, 79, 90], dtype=np.int64)
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # Scale by the pawn count of whichever side the evaluation currently
    # favours: it is that side which has to convert, and it cannot without
    # pawns. `total` is side-to-move relative, so a positive score means the
    # side to move is the one holding the advantage.
    if total > 0:
        strong_pawns = popcount(bb[WP]) if st[0] == _WHITE else popcount(bb[BP])
    elif total < 0:
        strong_pawns = popcount(bb[BP]) if st[0] == _WHITE else popcount(bb[WP])
    else:
        return total
    if strong_pawns < 6:
        total = (total * PAWN_SCALE_FACTOR[strong_pawns]) // 100
    return total
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                if (elapsed + iteration_ms * growth > soft_ms
                        and elapsed > soft_ms * PREDICTION_MIN_FRACTION):
                    break
