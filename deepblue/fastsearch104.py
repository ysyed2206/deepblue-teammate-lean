"""S1-search104: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Complexity-based time allocation (fastsearch104). Every time-management
# candidate tested so far read a signal AFTER spending the time -- did the best
# move change, did the score swing, how did the last iteration cost compare.
# All of them failed. This reads a signal that is available BEFORE the time is
# spent: how many legal moves the position actually has.
#
# A position with 45 legal replies genuinely needs more search than one with 8;
# a fixed per-move budget spends the same on both. The factor is deliberately
# narrow (0.85x to 1.25x) because this engine has already shown it punishes
# aggressive clock changes -- a 1.6x extension measured -39 Elo and a budget
# floor measured -64 Elo.
COMPLEXITY_MIN_FACTOR = 0.85
COMPLEXITY_MAX_FACTOR = 1.25
COMPLEXITY_PIVOT = 32.0   # legal moves at which the factor is exactly 1.0
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                completed = depth
                # Scale the soft budget by how branchy this position actually
                # is. legal is the root move count from the iteration just
                # finished, so this costs nothing extra to obtain.
                complexity = legal / COMPLEXITY_PIVOT
                if complexity < COMPLEXITY_MIN_FACTOR:
                    complexity = COMPLEXITY_MIN_FACTOR
                elif complexity > COMPLEXITY_MAX_FACTOR:
                    complexity = COMPLEXITY_MAX_FACTOR
                complexity_soft_ms = min(hard_ms * 0.9, soft_ms * complexity)
                ...
                if (elapsed + iteration_ms * growth > complexity_soft_ms
                        and elapsed > complexity_soft_ms * 0.5):
                    break
