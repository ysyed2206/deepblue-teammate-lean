...
FITTED KING-ATTACK WEIGHTS (fastsearch172, 2026-09-09). fastsearch140's only
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # Term scales refitted with the tuned attack weights in place
    # (tools/tune_terms.py). Validation MSE 0.031680 -> 0.030635. Danger fits
    # at 0.94 now, so the weights had been carrying the whole error.
    extra = (passed_pawns_white_relative(bb) * 18) // 100
    extra += king_attack_danger_tuned_white_relative(bb, phase, _TOTAL_PHASE)
    extra += (mobility_white_relative(bb) * 96) // 100
    ...
    extra += (bishop_pair_white_relative(bb) * 65) // 100
    ...
    # harmful on its own.
    extra += (rook_files_white_relative(bb) * 160) // 100
    extra += (knight_outposts_white_relative(bb) * 184) // 100
    if st[0] != _WHITE:
        extra = -extra
    return score + extra
