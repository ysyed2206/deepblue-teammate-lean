"""S1-search100: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Delta pruning (fastsearch100), stacked on fastsearch97's SEE pruning. Both
# target quiescence, which counting showed to be 59-71% of every node this
# engine searches, but they remove different moves: SEE removes captures that
# LOSE material, delta removes captures that cannot reach alpha even when they
# WIN material. Checked before the SEE test because it is far cheaper -- one
# piece-value lookup and an add, versus a full exchange evaluation -- so a move
# delta can reject never pays for SEE.
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for index in range(count):
        move = pick_best(pseudo[ply], scores[ply], count, index)
        # Delta pruning, tried first because it is the cheaper of the two
        # filters. Plain captures only: a promotion swings material by far more
        # than the captured piece is worth. This branch is never in check (the
        # in-check case returned earlier), so no escape from mate can be pruned.
