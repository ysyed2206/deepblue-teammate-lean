"""S1-search93: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Fraction of the soft budget that must be spent before the predictive stop
# is trusted to end the search. Was effectively 0.5 (fastsearch76); raised
# because measurement showed the predictor systematically over-estimates the
# next iteration and quits with a quarter to a half of the budget unused.
BUDGET_FLOOR_FRACTION = 0.85
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                # Budget utilisation floor (fastsearch93). Measured on real
                # positions from rounds 41-44, the predictive stop was leaving
                # 26-48% of the soft budget unspent AND reaching no extra
                # depth for it: given 4000 ms it used ~2900 ms and finished at
                # the same depth 10 it reached with a 2300 ms budget. Round
                # 44's losing move is the concrete cost -- at move 42 the
                # allocator granted 2299 ms, the search used 1590 ms and
                # stopped at depth 12, and depth 12 is the ONE depth between 9
                # and 14 at which this engine prefers the losing Qb8 over Bd6.
                #
                # The prediction is now only allowed to end the search once a
                # real fraction of the budget is actually spent. Starting an
                # iteration that does not finish is bounded: the hard-time
                # timer aborts it and the driver falls back to the last
                # COMPLETED iteration's move (`if legal == 0 or self.stop[0]:
                # break` runs before best_move is updated), so the downside is
                # spent clock, not a worse move -- while the upside is the
                # extra ply that round 44 needed.
                if (elapsed + iteration_ms * growth > soft_ms
                        and elapsed > soft_ms * BUDGET_FLOOR_FRACTION):
                    break
