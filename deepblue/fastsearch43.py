"""S1-search43: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Aspiration windows. After a few iterations the score is usually close to the
# last one, so searching the root with a narrow window around it lets far more
# subtrees fail high or low cheaply. When the true score does fall outside, the
# window is widened and the iteration re-searched -- so this trades a rare
# expensive re-search for a much cheaper common case. It is the classic way to
# cut effective branching factor without changing what the engine believes.
    ...
    for order_index in range(count):
        ...
        if legal == 0:
            ...
            score = -negamax(bb, occ, mail, st, depth - 1, -alpha - 1, -alpha, np.int64(1), child,
                             ...
                             tt_move_arr, killers, history,
                             piece_keys, side_key, castle_keys, ep_file_keys)
            if score > alpha and score < root_beta and not stop[0]:
                score = -negamax(bb, occ, mail, st, depth - 1, -root_beta, -alpha,
                             np.int64(1), child,
                                 pseudo, scores, undo, stop, counters, probe, path_hashes,
                                 game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                # Shallow iterations are cheap and their scores are volatile,
                # so those keep the full window; from ASPIRATION_MIN_DEPTH the
                # window narrows and only widens again on an actual fail.
                if depth >= ASPIRATION_MIN_DEPTH and completed > 0:
                    delta = ASPIRATION_DELTA
                    window_alpha = np.int64(best_score - delta)
                    window_beta = np.int64(best_score + delta)
                else:
                    delta = ASPIRATION_DELTA
                    window_alpha = np.int64(-INFINITY)
                    window_beta = np.int64(INFINITY)
                ...
                while True:
                    ...
                    # A score at or outside an edge is not a real bound: the
                    # true value lies beyond it, so widen that side and redo
                    # the iteration. Widening only the failing side keeps the
                    # other half of the window tight.
                    if iteration_score <= int(window_alpha) > -INFINITY:
                        delta *= 2
                        window_alpha = np.int64(max(-INFINITY, iteration_score - delta))
                        continue
                    if iteration_score >= int(window_beta) < INFINITY:
                        delta *= 2
                        window_beta = np.int64(min(INFINITY, iteration_score + delta))
                        continue
                    break
