"""S1-search117: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Razoring (fastsearch117). RFP asks whether the static eval is so far ABOVE
# beta that a fail-high can be assumed; razoring asks the mirror question --
# is it so far BELOW alpha that the node is probably hopeless. The difference
# is that razoring does NOT trust the static eval alone: it runs a quiescence
# search first and only returns if that VERIFIES the node is still below
# alpha.
#
# That verification is what makes it safe here. This engine has repeatedly
# rejected DISCARD techniques (LMP scored 5% -- not a typo) while accepting
# REDUCE-then-verify ones (null move, LMR, SEE pruning). Razoring is in the
# second category.
#
# Its recorded 60ms-era rejection is void: that gate ran at depth 4, where a
# depth<=3 condition barely fires and the quiescence verification dominates.
RAZOR_MAX_DEPTH = 3
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # Razoring: a hopeless-looking shallow non-PV node, VERIFIED by quiescence
    # before it is allowed to cut. Never at PV or check nodes.
    if (
        not is_pv
        and not checked
