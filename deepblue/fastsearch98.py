"""S1-search98: fastsearch4 + a transposition table probe/store inside qsearch.
...
def evaluate(bb, st, mg_table, eg_table, phase_table, alpha, beta):
    ...
    # Far outside the window: the expensive terms cannot change the outcome.
    if score + LAZY_EVAL_MARGIN <= alpha or score - LAZY_EVAL_MARGIN >= beta:
        return score
...
# Lazy evaluation (fastsearch98). Measured: a full evaluate() costs ~3635ns of
# which king_attack_danger is 41%, mobility 38% and passed_pawns 27%, and with
# ~0.8 evaluations per node at ~275k nps that is roughly 80% of this engine's
# entire runtime. The engine spends four fifths of its time judging positions
# and one fifth searching, which is why depth has been stuck at 8-10.
#
# So: compute the cheap material + piece-square score first, and only pay for
# the expensive terms when the position is close enough to the search window
# for them to change the answer.
#
# The margin is derived from measurement, not guessed. Across 1951 positions
# taken from our own real games the combined expensive terms never exceeded
# 170cp (median 27, p99 128). At 250 the cheap score cannot be wrong enough to
# cross the window: if base + 250 <= alpha then the true score, at most
# base + 170, is still below alpha.
LAZY_EVAL_MARGIN = np.int64(250)
...
def rfp_margin(bb, st, ply, is_pv, checked, depth, tt_move, original_alpha, beta,
    ...
    if (
        ...
        counters[RFP_TRY] += 1
        counters[EVAL] += 1
        # Lazy evaluation is deliberately DISABLED here. RFP compares
        # static_eval - margin against beta with margins up to 400cp, which
        # exceeds the lazy margin, so a cheap score could produce a cutoff the
        # true score does not justify. Every other call site compares against
        # the window directly and is safe.
        static_eval = evaluate(bb, st, mg_table, eg_table, phase_table,
                               np.int64(-INFINITY), np.int64(INFINITY))
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    if stop[0]:
        return 0
    if ply >= MAX_PLY - 2:
        return evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE, alpha, beta)
    ...
    counters[EVAL] += 1
    stand_pat = evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE, alpha, beta)
    counters[GEN] += 1
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if (
        ...
        if non_pawn != np.uint64(0):
            counters[EVAL] += 1
            nmp_eval = evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE, alpha, beta)
            if nmp_eval >= beta:
                old_side = st[0]
                old_ep = st[2]
                old_halfmove = st[3]
    ...
    if futility_active:
        counters[EVAL] += 1
        futility_eval = evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE, alpha, beta)
