"""S1-search112: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Node-effort time management (fastsearch112), taken from Ethereal's
# src/timeman.c, where it is the WIDEST of its three scaling factors
# (0.50-2.40, against 1.20-0.80 for best-move stability and 0.75-1.25 for
# score stability) and therefore the dominant signal.
#
# The question it answers is not "did the best move change" -- a label
# comparison that this engine already rejected at -39 Elo -- but "how much of
# this iteration's work went into moves OTHER than the one that won". If the
# alternatives were all refuted cheaply, the position is settled and the
# remaining budget is better saved. If effort is spread across many moves, it
# is genuinely contested and deserves more.
#
#     factor = 2 * non_best_fraction + 0.4      (Ethereal's own formula)
#
# An earlier attempt at this idea (fastsearch89) used a narrower 0.6-1.5 range
# of my own choosing and, worse, was tested in --mode time, where saved clock
# simply evaporates and the candidate is quietly allowed to spend past the
# baseline's budget. It read +107 Elo for that reason alone and was never
# validly measured. This is the researched formula, tested in clock mode.
NODE_EFFORT_MIN_DEPTH = 6
NODE_EFFORT_FACTOR_MIN = 0.50
NODE_EFFORT_FACTOR_MAX = 2.40
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
                ...
                killers, history, continuation_history, prev_moves,
                piece_keys, side_key, castle_keys, ep_file_keys,
                root_moves, root_scores, root_move_nodes,
                previous_best, root_alpha, root_beta):
    side = st[0]
    path_hashes[0] = value
    ...
    for order_index in range(count):
        ...
        nodes_before_move = counters[NODES]
        old_castle = st[1]
        old_ep_file = Z.canonical_ep_file(bb, occ, st)
        ...
        root_move_nodes[legal] = counters[NODES] - nodes_before_move
    ...
    if legal > 0:
        ...
        if best != 0:
            ...
            root_move_nodes[0], root_move_nodes[best] = (
                root_move_nodes[best], root_move_nodes[0])
...
class FastEngine112:
    ...
    def __init__(self) -> None:
        ...
        self.root_move_nodes = np.zeros(MAX_MOVES, dtype=np.int64)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.root_moves, self.root_scores, self.root_move_nodes,
                        np.uint32(best_move),
                ...
                completed = depth
                effort_total = 0
                for _i in range(legal):
                    effort_total += int(self.root_move_nodes[_i])
                if effort_total > 0:
                    non_best = 1.0 - int(self.root_move_nodes[0]) / effort_total
                else:
                    non_best = 0.0
                effort_factor = 2.0 * non_best + 0.4
                if effort_factor < NODE_EFFORT_FACTOR_MIN:
                    effort_factor = NODE_EFFORT_FACTOR_MIN
                elif effort_factor > NODE_EFFORT_FACTOR_MAX:
                    effort_factor = NODE_EFFORT_FACTOR_MAX
                if depth >= NODE_EFFORT_MIN_DEPTH:
                    effort_soft_ms = min(hard_ms * 0.9, soft_ms * effort_factor)
                else:
                    effort_soft_ms = soft_ms
                ...
                if (elapsed + iteration_ms * growth > effort_soft_ms
                        and elapsed > effort_soft_ms * 0.5):
                    break
