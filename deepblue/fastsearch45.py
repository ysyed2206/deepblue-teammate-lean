"""S1-search45: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Delta pruning (qsearch). A capture cannot possibly raise alpha if even
# its best case -- winning the captured piece outright, plus a safety
# margin for endgame/positional swing -- still lands below alpha. Skipping
# it is the principled version of the hard qsearch movecount/depth caps
# bolted onto fastsearch30 this morning to survive the Kiwipete explosion;
# this prunes by the actual evaluation gap instead of an arbitrary count,
# so it should never discard a capture that could genuinely still matter.
# Not applied in the endgame (DELTA_PRUNE_PHASE_FLOOR) or near mate scores,
# where centipawn margins stop being meaningful.
DELTA_MARGIN = 150
DELTA_PRUNE_PHASE_FLOOR = 4
CAPTURE_VALUE_CP = np.array(
    [100, 320, 330, 500, 900, 20000, 100, 320, 330, 500, 900, 20000], dtype=np.int32
)
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for piece_index in range(12):
        bits_p = bb[piece_index]
        while bits_p:
            bits_p &= bits_p - np.uint64(1)
            phase += PHASE_TABLE[piece_index]
    delta_prune_active = phase >= DELTA_PRUNE_PHASE_FLOOR and abs(alpha) < MATE_THRESHOLD

    for index in range(count):
        move = pick_best(pseudo[ply], scores[ply], count, index)
        # Delta pruning: a capture whose best case -- the captured piece's
        # full value, plus a margin for tactics the static eval can't see --
        # still can't reach alpha is not worth a make/unmake, let alone a
        # recursive search. Promotions are exempt: their value is the
        # promoted piece, not the (usually small) captured one, so the same
        # arithmetic would prune them on the wrong number.
        captured_piece = (move >> np.uint32(16)) & np.uint32(15)
        promotion_piece = (move >> np.uint32(20)) & np.uint32(15)
        if (
            delta_prune_active
            and promotion_piece == np.uint32(NO_PIECE)
            and captured_piece != np.uint32(NO_PIECE)
            and stand_pat + CAPTURE_VALUE_CP[captured_piece] + DELTA_MARGIN <= alpha
        ):
            continue
        old_castle = st[1]
        old_ep_file = Z.canonical_ep_file(bb, occ, st)
