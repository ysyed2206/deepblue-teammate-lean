"""S1-search37: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late move reduction: when to start reducing, and by how much.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVES = 3
LMR_BASE = 1
LMR_WIDE_MOVES = 6
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            # Late Move Reduction. Late quiet moves get their null-window probe
            # at REDUCED depth; anything that unexpectedly beats alpha is then
            # re-searched at full depth. This is the crucial difference from
            # LMP, which discards late quiet moves entirely and measured 6.2%
            # here: LMR never throws a move away unseen, it only looks at the
            # unlikely ones more cheaply first.
            ...
            if (
                ...
                counters[LMR_TRY] += 1
                reduction = LMR_BASE
                if legal > LMR_WIDE_MOVES:
                    reduction += np.int64(1)
                if depth >= LMR_DEEP_DEPTH:
                    reduction += np.int64(1)
                # Never reduce into quiescence: keep at least one real ply.
