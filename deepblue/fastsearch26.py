"""S1-search26: fastsearch18 + bounded gravity with a donor bonus curve.

History decomposition candidate #3 of 3 (priority #2). Parent: fastsearch18
(qsearch TT), independent of candidate #1 (fastsearch24, butterfly
indexing) and candidate #2 (fastsearch25, symmetric malus) -- built on
fastsearch18's ORIGINAL piece-to (12,64) table, not stacked on either.
Attribution to fastsearch18's own qsearch-TT changes is preserved verbatim
below.

This candidate deliberately combines two closely-related sub-pieces from
the decomposition list (gravity + bonus-curve) into one candidate, per
explicit session direction naming it "bounded gravity + donor bonus
curve" as a single unit -- a small, acknowledged deviation from the
strict one-idea-per-candidate rule, noted here rather than silently. If
this candidate's result is ambiguous, the two pieces should be split
before drawing further conclusions.

Donor research (DONOR_NEXT_IDEAS.md): gravity is the ONLY decay mechanism
any donor uses (no separate aging pass), universal form
``entry += bonus - entry * |bonus| / MAX``, self-bounding toward +-MAX
with no explicit clamp needed for a bonus-only (no-malus) update --
verified below to hold, plus a defensive clamp added anyway per this
codebase's own "watch for signed integer floor/truncation differences"
guidance. ``MAX_HISTORY = 16384`` is the majority donor constant
(Pawnstar, Ethereal, Coda) and, per AC-003, was NOT what made the
rejected internal Patch A fail (Patch A's failure was an extra decay pass
on top of gravity, plus a flat 5x asymmetric malus -- neither present
here; this candidate has no malus at all).

Bonus curve: Pawnstar's ``min(depth*depth, 400)`` -- chosen over the
linear-capped Stockfish/Coda/Viridithas shape specifically because it is
the SMALLEST change from fastsearch18's existing raw ``depth*depth`` (adds
only a cap), keeping this already-combined candidate's second axis as
conservative as possible rather than compounding two unfamiliar shapes at
once.

No malus (that is candidate #2, fastsearch25, tested independently).
...
# History gravity + donor bonus curve (Pawnstar shape): majority donor
# constant for the saturation scale, and the smallest cap change from the
# existing raw depth*depth bonus.
MAX_HISTORY = 16384
HISTORY_BONUS_CAP = 400
    ...
    for order_index in range(count):
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                piece = (move >> np.uint32(12)) & np.uint32(15)
                to_square = (move >> np.uint32(6)) & np.uint32(63)
                raw_bonus = depth * depth
                bonus = raw_bonus if raw_bonus < HISTORY_BONUS_CAP else HISTORY_BONUS_CAP
                entry = history[piece, to_square]
                updated = entry + bonus - entry * bonus // MAX_HISTORY
                if updated > MAX_HISTORY:
                    updated = MAX_HISTORY
                elif updated < -MAX_HISTORY:
                    updated = -MAX_HISTORY
                history[piece, to_square] = updated
            break
        if stop[0]:
            break
