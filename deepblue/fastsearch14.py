"""S1-search14: fastsearch4 + TT move preservation on null-best writes.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if tt_usable and not stop[0]:
        ...
        if tt_depth[index] <= depth or tt_key[index] != value:
            # Preserve a useful move already stored for the same position when
            # this node has no best move of its own (typically an all-node).
            # Otherwise an upper-bound write silently erases future TT ordering.
            if best_move == np.uint32(0) and tt_key[index] == value:
                counters[P1A_CANDIDATE] += 1
                if tt_move_arr[index] != np.uint32(0):
                    counters[P1A_FIRE] += 1
