"""S1-search54: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late move reductions, using Ethereal's own published, battle-tested
# constants (chessprogramming.org/Late_Move_Reductions) rather than
# invented ones -- LMR failed 4 times in this codebase with home-grown
# formulas (55.2%, 28.5%, 40.4%, and an earlier inconclusive ~51.9%), and
# the diagnosis each time was miscalibration, not that the technique itself
# is unsound. Ethereal's formula: quiet moves reduce by
# 0.7844 + ln(depth)*ln(moves)/2.4696; captures reduce by a flat 3, or 2 if
# the move gives check. This is the first attempt here to also reduce
# captures, which was never tried in the four prior failures.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVE = 2
        ...
        if legal == 1:
            ...
            if (
                ...
                if move_captured0 == np.uint32(NO_PIECE) and move_promotion0 == np.uint32(NO_PIECE):
                    ...
                    reduction = np.int64(2) if gives_check else np.int64(3)
