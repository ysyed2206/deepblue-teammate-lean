"""S1-search49: fastsearch4 + a transposition table probe/store inside qsearch.
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                # time_manager.py's own docstring: soft is "the point at which
                # we stop STARTING a new pass"; hard is "the point at which we
                # abandon the pass in PROGRESS". That already grants an
                # in-progress pass the soft-to-hard margin -- but this
                # predictive check compared the NEXT pass's estimate against
                # soft_ms, refusing to even start an iteration the design's
                # own contract would have let run into that margin. Measured:
                # actual spend was ~53% of the allocated soft_ms budget in
                # real games. Comparing against hard_ms instead lets a
                # predicted-to-be-worthwhile iteration start whenever it
                # would not be discarded outright by the hard cutoff; a
                # genuinely wasted iteration (predicted past hard_ms) is
                # still refused exactly as before.
                if elapsed + iteration_ms * growth > hard_ms:
                    break
