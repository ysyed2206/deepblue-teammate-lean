"""S1-search41: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.see import see_value  # noqa: E402
...
# A capture that loses material even in the best case does not deserve a slot
# ahead of killers and history. Ordering it with the winning captures wastes
# the earliest, most valuable positions in the move list -- the ones that
# decide how quickly alpha-beta gets a cutoff, and therefore the effective
# branching factor. Measured EBF here is ~5.3 against ~2.5 for a strong
# engine, and that exponent costs more than any evaluation term.
ORDER_BAD_CAPTURE = -(1 << 20)
...
def order_moves(moves, count, scores, mail, tt_move, killers, history, ply,
    ...
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # see_value must not be used for promotions; this branch is
            # reached only after the promotion case above has been handled.
            if see_value(bb, occ, mail, st, move) < 0:
                scores[index] = ORDER_BAD_CAPTURE + mvv_lva
            else:
                scores[index] = ORDER_CAPTURE + mvv_lva
