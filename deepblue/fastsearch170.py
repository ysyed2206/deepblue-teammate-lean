...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # TUNED TERM SCALES (fastsearch170, 2026-09-09).
    #
    # Every weight in this evaluation was hand-picked and then judged by a
    # self-play match that resolves about +/-28 Elo -- too coarse to tell a
    # correct weight from one that is three times too large. These eight
    # scales are instead fitted to 120k quiet positions carrying deep
    # Stockfish scores (tools/tune_terms.py), with base_evaluate pinned at
    # 1.0 so the numbers read as "worth this much material". Validation MSE
    # on a held-out 20% fell 0.032198 -> 0.030607.
    #
    # The two large corrections are king safety, and they are corroborated by
    # two games rather than by the fit alone:
    #
    #   danger x0.33  Round 80 built an attack (Qh5, f4, g4) our eval scored
    #                 +90 and a 45s search scores -77. We overpay for our own
    #                 attacks and then have to walk them back.
    #   shield x0     Contributes nothing measurable against Stockfish. Note
    #                 this is NOT "king safety does not matter" -- round 77
    #                 shows the opposite. It is that THIS term, counting our
    #                 own pawns near our own king, does not carry the signal.
    #
    # Scales are applied as integer ratios; no term is recomputed, so this
    # costs no nodes per second and no Numba compile time -- which matters
    # because 162 already reads 1.42x fastsearch140's cold start.
    extra = (passed_pawns_white_relative(bb) * 18) // 100
    extra += (king_attack_danger_strongq_white_relative(bb, phase, _TOTAL_PHASE) * 33) // 100
    extra += (mobility_white_relative(bb) * 95) // 100
    ...
    extra += (bishop_pair_white_relative(bb) * 64) // 100
    ...
    extra += (pawn_structure_white_relative(bb) * 151) // 100
    ...
    # harmful on its own.
    extra += (rook_files_white_relative(bb) * 162) // 100
    extra += (knight_outposts_white_relative(bb) * 194) // 100
    if st[0] != _WHITE:
        extra = -extra
    return score + extra
