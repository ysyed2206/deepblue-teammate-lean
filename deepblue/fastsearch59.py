"""S1-search59: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Singular extension (donor: Ethereal's singularity()/search.c, read from
# its actual GitHub source). Trigger: depth >= 8, the TT move is a
# recorded fail-high (LOWER bound) from a search that went at least
# depth-3 deep. SINGULAR_DOUBLE_MARGIN/SINGULAR_MAX_DEXTENSIONS match
# Ethereal's own literal constants (16 and 6).
SINGULAR_MIN_DEPTH = 10
SINGULAR_TT_DEPTH_MARGIN = 3
SINGULAR_DOUBLE_MARGIN = 16
SINGULAR_MAX_DEXTENSIONS = 6
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, excluded_move, dextensions,
            piece_keys, side_key, castle_keys, ep_file_keys):
    counters[NODES] += 1
    if stop[0]:
        return 0
    ...
    if tt_usable and tt_key[index] == value:
        ...
        tt_hit_bound = tt_bound[index]
        tt_hit_score = score_from_tt(tt_score[index], ply)
        if tt_hit_depth >= depth and ply > 0:
            stored = tt_hit_score
            bound = tt_hit_bound
            if bound == EXACT:
                return stored
    ...
    for order_index in range(count):
        ...
        if (
            order_index == 0
            and move == tt_move
            and move != np.uint32(0)
            and excluded_move == np.uint32(0)
            and depth >= SINGULAR_MIN_DEPTH
            and tt_hit_depth >= depth - SINGULAR_TT_DEPTH_MARGIN
            and tt_hit_bound == LOWER
        ):
            # Singular extension. The TT move is already a recorded
            # fail-high; verify that by searching every OTHER move at this
            # exact position (TT move excluded) to a reduced depth with a
            # narrow window just below its recorded value. If nothing else
            # comes close, the TT move is "singular" and earns extra depth;
            # if something else meets the bar, this whole node is itself a
            # multi-cut candidate and can be resolved immediately.
            r_beta = tt_hit_score - depth
            if r_beta < -MATE_SCORE:
                r_beta = -MATE_SCORE
            verify_depth = (depth - 1) // 2
            verify_score = -negamax(
                bb, occ, mail, st, verify_depth, -r_beta, -(r_beta - 1), ply, value,
                pseudo, scores, undo, stop, counters, probe, path_hashes,
                game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                tt_move_arr, killers, history, move, dextensions,
                piece_keys, side_key, castle_keys, ep_file_keys)
            # That call generated its own moves into pseudo[ply]/scores[ply]
            # -- the same scratch slot this node's own loop is still reading
            # from, since every ply has exactly one shared buffer, not one
            # per recursion. Regenerate before resuming: same position, so
            # this reproduces the identical list, order and count.
            count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
            order_moves(pseudo[ply], count, scores[ply], mail, tt_move, killers, history, ply)
            move = pick_best(pseudo[ply], scores[ply], count, 0)
            if not stop[0]:
                if verify_score >= r_beta and r_beta >= beta:
                    return r_beta
                if (
                    not is_pv
                    and verify_score < r_beta - SINGULAR_DOUBLE_MARGIN
                    and dextensions <= SINGULAR_MAX_DEXTENSIONS
                ):
                    extra_extension = 2
                elif verify_score < r_beta:
                    extra_extension = 1
                elif tt_hit_score >= beta:
                    extra_extension = -1
                elif tt_hit_score <= alpha:
                    extra_extension = -1
        ...
        child_depth = depth - 1 + (CHECK_EXTENSION if checked else 0) + extra_extension
        child_dextensions = dextensions + 1 if extra_extension == 2 else dextensions
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, np.uint32(0), child_dextensions,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
    ...
    for order_index in range(count):
        ...
        if legal == 0:
            score = -negamax(bb, occ, mail, st, depth - 1, np.int64(-INFINITY), -alpha,
                             ...
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, np.uint32(0), 0,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
