"""S1-search92: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Razoring (fastsearch92). RFP asks "is the static eval so far ABOVE beta
# that we can assume a fail-high"; razoring asks the mirror question -- is it
# so far BELOW alpha that this node is probably hopeless. The difference from
# RFP is that razoring does NOT trust the static eval on its own: it runs a
# quiescence search first and only returns if that VERIFIES the node is still
# below alpha. That verification is what makes it safe here, where the flat
# discard technique (LMP) previously failed at 60ms-era testing because move
# ordering is weak -- this is a reduce-then-verify technique, the category
# that has actually worked in this engine (NMP, LMR).
# Expected to matter most in endgames, which are full of quiet hopeless
# nodes and where the branching factor is low enough that the saved nodes
# convert directly into depth.
RAZOR_MAX_DEPTH = 3
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # Razoring: hopeless-looking shallow non-PV node, VERIFIED by quiescence
