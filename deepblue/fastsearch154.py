"""S1-search154: fastsearch4 + a transposition table probe/store inside qsearch.
...
def qsearch_tt_store(tt_usable, stopped, best, original_alpha, beta, tt_index,
                      tt_key, tt_score, tt_depth, tt_bound, tt_move_arr, tt_age, tt_gen,
    ...
    if tt_usable and not stopped:
        ...
        if (tt_depth[tt_index] <= 0 or tt_key[tt_index] != value
                or tt_age[tt_index] != tt_gen):
            tt_age[tt_index] = tt_gen
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
               probe, tt_key, tt_score, tt_depth, tt_bound, tt_move_arr, tt_age, tt_gen,
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr, tt_age, tt_gen,
            killers, history, continuation_history, prev_moves, corr_hist,
            piece_keys, side_key, castle_keys, ep_file_keys,
    ...
    if depth <= 0:
        return quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop,
                          counters, probe, tt_key, tt_score, tt_depth, tt_bound, tt_move_arr, tt_age, tt_gen,
                          piece_keys, side_key, castle_keys, ep_file_keys)
    ...
    if (
        ...
        if razor_eval + RAZOR_MARGIN_BASE + RAZOR_MARGIN_PER_DEPTH * depth <= alpha:
            razor_score = quiescence(
                ...
                stop, counters, probe, tt_key, tt_score, tt_depth, tt_bound, tt_move_arr, tt_age, tt_gen,
                piece_keys, side_key, castle_keys, ep_file_keys)
            if stop[0]:
                return 0
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             ...
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, tt_age, tt_gen,
                             killers, history, continuation_history,
                             prev_moves, corr_hist,
                             piece_keys, side_key, castle_keys, ep_file_keys,
                             False, True)
        else:
            reduction = 0
    ...
    if tt_usable and not stop[0]:
        if best <= original_alpha:
            ...
            bound = EXACT
        # Replace on a different position, an equal-or-deeper search, OR an
        # entry from an earlier move. Without that last clause a deep entry
        # from twenty moves ago outranks everything and never leaves the
        # table -- measured: 76% of occupied slots are stale after 5 moves.
        if (tt_depth[index] <= depth
                or tt_key[index] != value
                or tt_age[index] != tt_gen):
            tt_age[index] = tt_gen
            tt_key[index] = value
...
class FastEngine154:
    ...
    def __init__(self) -> None:
        ...
        self.tt_age = np.zeros(TT_SIZE, dtype=np.uint8)
        self.tt_generation = np.uint8(0)
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        self.killers[:] = 0
        self.prev_moves[:] = 0
        self.tt_generation = np.uint8((int(self.tt_generation) + 1) & 0xFF)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.tt_age, self.tt_generation,
