"""S1-search19: fastsearch4 with the quiet-TT-move RFP guard A/B'd away.

Priority-list item #7. fastsearch4's RFP gate requires, among other things,
``not tt_move_is_quiet`` -- it refuses to prune when the stored TT move for
this node is a non-capture/non-promotion, on the theory that a quiet TT move
is evidence static eval alone may be hiding the only good defence. That guard
is fastsearch4's own addition, not part of the donor formula our research
notes cite (Pawnstar's public RFP evidence is just "non-PV, depth<=7,
static_eval - 80*depth >= beta", no TT-move-quietness condition at all).

This candidate removes exactly that one guard and nothing else, to measure
in isolation whether it is earning its keep or just costing prunes. Every
other line is byte-identical to fastsearch4.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # restricted to non-PV, non-check, interior, shallow nodes.
    #
    # fastsearch19 / P7 A/B: fastsearch4's quiet-TT-move guard is removed
    # here on purpose -- this candidate prunes even when the stored TT move
    # is quiet, to measure whether that guard was earning its keep.
