"""S1-search103: fastsearch4 + a transposition table probe/store inside qsearch.
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # by where it stands, so two pawns stacked on a file score as well as
    # two abreast. These are structural weaknesses the search cannot find
    # tactically -- they cost material far beyond the horizon -- which is
    # exactly the shape of losing gradually rather than to a blunder.
