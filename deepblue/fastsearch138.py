"""S1-search138: fastsearch4 + a transposition table probe/store inside qsearch.
...
# History-modulated late move reductions. We already compute a history score
# for every quiet move to ORDER it, then discard that information when
# deciding how far to reduce -- so a move the history strongly likes is
# reduced exactly as hard as one it has never seen succeed.
#
# Standard practice in modern engines: reduce less for moves with good
# history, more for moves with bad history. Stockfish derives its reduction
# adjustment from a combined stat score over main and continuation histories.
#
# HISTORY_MAX is 16384, so a divisor of 8192 gives roughly +/-2 plies from the
# main table before the continuation table is added; the result is clamped to
# +/-2 so a single saturated entry cannot cancel the reduction entirely or
# double it.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if (
        ...
        counters[EVAL] += 1
        razor_eval = evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE)
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            if (
                ...
                # Reduce less for a move this position's history likes.
