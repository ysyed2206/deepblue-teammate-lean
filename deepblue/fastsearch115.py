"""S1-search115: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Contempt (fastsearch115). DRAW_SCORE is 0, so a repetition scores exactly
# equal to an unclear position and the engine takes a draw whenever its
...
# -15 -- by its own arithmetic the draw (0) was better, so it repeated on
# purpose. In a Swiss where the team has to climb, an early draw as White is
# nearly a loss.
#
# Applied ONLY to repetition draws. Stalemate, insufficient material and the
# fifty-move rule are genuinely drawn positions and must keep scoring exactly
# 0 -- a first attempt at this replaced all thirteen DRAW_SCORE sites and
# failed four regression positions that assert a terminal draw scores 0.
# Kept small: too much contempt makes the engine decline draws it should take
# in worse positions, and lose instead.
