"""S1-search58: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Capture history: a separate table from the quiet-move history above,
# indexed by (moving piece, captured piece, to-square) rather than just
# (piece, to-square), since a capture's value depends on what it takes.
# Deliberately capped far below ORDER_CAPTURE's own bucket width (roughly
# -4..319 of pure MVV-LVA spread) so it can re-rank captures that are
# already close under MVV-LVA, and even occasionally overturn a distant
# one that has repeatedly cut off, without ever pushing a capture's score
# into the promotion or TT-move buckets.
CAPTURE_HISTORY_CEILING = 256
...
@njit(cache=True, nogil=True)
def order_moves(moves, count, scores, mail, tt_move, killers, history, capture_history, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            bonus = capture_history[piece, captured, to_square]
            if bonus > CAPTURE_HISTORY_CEILING:
                bonus = CAPTURE_HISTORY_CEILING
            scores[index] = ORDER_CAPTURE + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece] + bonus
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, capture_history, piece_keys, side_key, castle_keys, ep_file_keys):
    counters[NODES] += 1
    if stop[0]:
        return 0
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    order_moves(pseudo[ply], count, scores[ply], mail, tt_move, killers, history, capture_history, ply)
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             ...
                             tt_move_arr, killers, history, capture_history,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                piece = (move >> np.uint32(12)) & np.uint32(15)
                to_square = (move >> np.uint32(6)) & np.uint32(63)
                capture_history[piece, captured, to_square] += depth * depth
            break
        if stop[0]:
            break
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history, capture_history, 0)
...
class FastEngine58:
    ...
    def __init__(self) -> None:
        ...
        # (piece, captured, to-square) -- persists across moves within a
        # game exactly like self.history above; never reset in search().
        self.capture_history = np.zeros((12, 12, 64), dtype=np.int32)
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                legal = search_root(
                    ...
                    self.killers, self.history, self.capture_history,
