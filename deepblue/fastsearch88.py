"""S1-search88: fastsearch4 + a transposition table probe/store inside qsearch.
...
NMP_MIN_DEPTH = 4
NMP_REDUCTION = 3  # retained for reference; superseded by the adaptive form below
NMP_BASE = 4
NMP_DEPTH_DIVISOR = 5
NMP_EVAL_DIVISOR = 191
NMP_EVAL_MAX = 3
    ...
    if (
        ...
        if non_pawn != np.uint64(0):
            ...
            if nmp_eval >= beta:
                ...
                # Ethereal's real adaptive reduction, read from its source.
                eval_margin = (nmp_eval - beta) // NMP_EVAL_DIVISOR
                if eval_margin > NMP_EVAL_MAX:
                    eval_margin = NMP_EVAL_MAX
                if eval_margin < 0:
                    eval_margin = 0
                reduction_r = NMP_BASE + depth // NMP_DEPTH_DIVISOR + eval_margin
                null_depth = depth - reduction_r
                if null_depth < 0:
                    null_depth = 0
                null_score = -negamax(
