"""S1-search152: fastsearch4 + a transposition table probe/store inside qsearch.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if tt_usable and not stop[0]:
        if best <= original_alpha:
            ...
            bound = EXACT
        # Replace when the slot holds a different position, when this search
        # is at least as deep, OR when the entry is from an earlier move. That
        # last clause is the whole point: without it a deep entry from twenty
        # moves ago outranks everything and never leaves the table.
...
class FastEngine152:
    ...
    def __init__(self) -> None:
        ...
        # generation of the search that wrote each entry, so stale deep
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        self.killers[:] = 0
        self.prev_moves[:] = 0
        # New generation for every MOVE, not every deepening iteration -- that
        # is what the replacement literature specifies. uint8 wraps after 256
        # moves; a wrapped match is benign (it merely makes one stale entry
        # look current again, which is the behaviour we have today anyway).
