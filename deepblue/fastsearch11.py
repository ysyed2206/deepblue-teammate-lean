"""S1-search11: accepted PVS+RFP baseline plus ordering-only Static Exchange Evaluation.
...
SEE is used only to rank captures. No SEE pruning, aspiration windows, null move, LMR, futility or IIR are present.
"""
...
# Ordering bands. SEE changes only the relative placement of captures: sound
# captures stay ahead of quiets, while losing captures are searched after all
# killer/history quiets. Promotions remain tactical-good unconditionally.
ORDER_TT = 1 << 27
ORDER_PROMOTION = 1 << 25
ORDER_GOOD_CAPTURE = 1 << 24
ORDER_KILLER_1 = 1 << 23
ORDER_KILLER_2 = (1 << 23) - 1
HISTORY_CEILING = (1 << 21) - 1
ORDER_BAD_CAPTURE = -(1 << 22)
ORDER_CAPTURE = 1 << 21  # qsearch keeps the accepted baseline ordering in Patch 06
...
def order_moves(bb, occ, mail, st, moves, count, scores, tt_move, killers, history, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # Promotions are never demoted by v1 SEE; promotion recapture trees
            # need special underpromotion handling and are too rare to justify
            # risking a bad ordering decision here.
            scores[index] = ORDER_PROMOTION + PIECE_VALUE[promotion] * 16
        elif captured != np.uint32(NO_PIECE):
            exchange = see_value(bb, occ, mail, st, move)
            mvv_lva = PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
            if exchange >= 0:
                scores[index] = ORDER_GOOD_CAPTURE + mvv_lva + exchange
            else:
                scores[index] = ORDER_BAD_CAPTURE + mvv_lva + exchange
