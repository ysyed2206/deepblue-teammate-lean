"""S1-search95: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Singular extensions (fastsearch95). Gated at depth 8, which is why the
# 60ms-era 48.1% rejection measured nothing: that gate ran at depth 4.
SINGULAR_MIN_DEPTH = 8
SINGULAR_TT_DEPTH_SLACK = 3   # TT entry must be within this of our own depth
SINGULAR_MARGIN = 2           # centipawns per ply below the TT score
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, excluded_moves,
            piece_keys, side_key, castle_keys, ep_file_keys,
    ...
    tt_move = np.uint32(0)
    excluded = excluded_moves[ply]
    tt_hit_score = np.int64(0)
    tt_hit_depth = np.int64(-1)
    tt_hit_bound = np.int64(-1)
    counters[TTPROBE] += 1
    if tt_usable and tt_key[index] == value:
        ...
        # A node searching under an exclusion must not take the cutoff: the
        # stored entry is exactly the claim being tested, so returning it
        # would make every move look singular.
        if tt_depth[index] >= depth and ply > 0 and excluded == np.uint32(0):
    ...
    # Singular extension test. Placed here deliberately: after the TT probe
    # (which supplies tt_move and its score) but BEFORE this node fills
    # pseudo[ply], because the verification search runs at this same ply and
    # writes those same scratch buffers. Nothing to clobber yet.
    singular_extension = 0
    if (
        excluded == np.uint32(0)
        and ply > 0
        and depth >= SINGULAR_MIN_DEPTH
        and tt_move != np.uint32(0)
        and tt_hit_depth >= depth - SINGULAR_TT_DEPTH_SLACK
        and tt_hit_bound == LOWER
        and abs(tt_hit_score) < MATE_THRESHOLD
    ):
        singular_beta = tt_hit_score - SINGULAR_MARGIN * depth
        excluded_moves[ply] = tt_move
        # Same position, same side to move -- a sibling test, not a child, so
        # the result is NOT negated.
        singular_score = negamax(
            bb, occ, mail, st, (depth - 1) // 2,
            ...
            tt_move_arr, killers, history, excluded_moves,
            piece_keys, side_key, castle_keys, ep_file_keys,
            True, False)
        excluded_moves[ply] = np.uint32(0)
    ...
    if tt_usable and not stop[0] and excluded == np.uint32(0):
        if best <= original_alpha:
            bound = UPPER
        elif best >= beta:
            bound = LOWER
...
class FastEngine95:
    ...
    def __init__(self) -> None:
        ...
        # Move barred from the search at each ply, for singular verification.
        self.excluded_moves = np.zeros(MAX_PLY, dtype=np.uint32)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.killers, self.history, self.excluded_moves,
