"""S1-search27: fastsearch18 + LMR-1 (bare late move reductions, "exempt" style).

Priority #4 on the roadmap ("LMR only after ordering is healthy"), first
slice per DONOR_NEXT_IDEAS.md's LMR decomposition. Parent: fastsearch18
(this session's confirmed-clean working baseline, no HIGH/CRITICAL
red-team findings) -- deliberately NOT stacked on the still-HOLD SEE
(fastsearch21/22) or history (fastsearch24/25/26) candidates, per the
"don't stack an unconfirmed candidate" rule.

Donor research (Agent A, DONOR_NEXT_IDEAS.md): Reckless's 2023-era first
LMR commit (exempt style) and its very first commit's 3-tier re-search
structure are the model here, both independently confirmed twice in
Reckless's own history (2023 era and the 2025 ground-up rebuild) to land
AFTER the full move-ordering stack (TT move, MVV-LVA, history, killers)
was already in place -- Deep Blue already has all of that.

Guards, matching donor's earliest ("exempt") style rather than the
loosened modern one, since Deep Blue's own ordering is far less mature
than current Reckless's: ``depth >= 3``, this is at least the 4th legal
move examined at this node (``legal >= 4`` -- three moves unreduced, not
donor's current ``>= 2``), the move is quiet (no capture, no promotion),
the SIDE TO MOVE at this node is not in check (not "the move gives
check" -- that is a separate, later refinement), and the move is not the
TT move. Fixed R=1 -- even more conservative than any donor's actual
first version (which already scaled with depth/move-count).

Re-search structure, the donor's documented 3-tier form (this is the
single most catastrophic thing to get wrong per donor failure-mode
research -- re-searching at the full window instead of null window first
is flagged as a classic, expensive error):

1. Null-window probe at REDUCED depth (``depth - 1 - R``).
2. If that fails high (``score > alpha``) AND a reduction was actually
   applied, re-verify at full depth, SAME null window (not full window).
3. Only if that also lands inside ``(alpha, beta)`` does the existing PVS
   full-window re-search (already present in fastsearch18/fastsearch4)
   fire, unchanged.

Not combined with LMR-2 (log reduction formula), LMR-3 (PV relief),
LMR-4 (killer relief), or any later LMR piece -- those are separate,
later candidates per the one-idea-per-candidate rule and the explicit
LMR dependency chain in DONOR_NEXT_IDEAS.md.
...
# LMR-1: bare, exempt-style late move reductions. Fixed R=1, deliberately
# more conservative than any donor's own first version (see docstring).
LMR_MIN_DEPTH = 3
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            # LMR-1: reduce a quiet, non-TT move late in a sufficiently deep,
            # non-check node. Fixed R=1; see module docstring for the 3-tier
            # re-search structure this feeds into (reduced null-window probe
            # -> full-depth null-window re-verify on surprise -> existing PVS
            # full-window re-search, unchanged, only if that also surprises).
            move_captured = (move >> np.uint32(16)) & np.uint32(15)
            move_promotion = (move >> np.uint32(20)) & np.uint32(15)
            reduction = 0
            if (
                depth >= LMR_MIN_DEPTH
                and legal >= LMR_MIN_MOVE_COUNT
                and move_captured == np.uint32(NO_PIECE)
                and move_promotion == np.uint32(NO_PIECE)
                and not checked
                and move != tt_move
            ):
            ...
            score = -negamax(bb, occ, mail, st, search_depth, -alpha - 1, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history,
                             piece_keys, side_key, castle_keys, ep_file_keys)
            if reduction > 0 and score > alpha and not stop[0]:
                counters[LMR_RESEARCH] += 1
...
class FastEngine27:
    ...
    def __init__(self) -> None:
        ...
        self.counters = np.zeros(18, dtype=np.int64)
