"""S1-search24: fastsearch18 + butterfly (side x from x to) history indexing.

History decomposition candidate #1 of 3 (priority #2). Parent: fastsearch18
(qsearch TT), used as the experimental foundation per current session
direction; attribution to fastsearch18's own qsearch-TT changes is
preserved verbatim below.

AC-003 (DEEPBLUE_AUDIT_CORRECTIONS.md) found that Deep Blue's history table
was never actually a "butterfly" (side x from-square x to-square) table --
it is piece-to: ``history[piece, to_square]``, shape (12, 64), no
from-square axis at all. A prior donor-research pass had wrongly claimed
this already matched Pawnstar's from-to butterfly convention. This
candidate is the correction, tested in isolation: ONLY the table's
indexing is changed, from ``(12, 64)`` piece-to to ``(2, 64, 64)``
side x from x to. The update mechanism is otherwise UNCHANGED from
fastsearch18 -- still the raw, unbounded ``+= depth * depth`` accumulation
on a cutoff, no malus, no gravity/clamping. This isolates the indexing
question from every other history idea (malus, gravity, bonus-curve are
separate candidates #2 and #3, each independently parented on fastsearch18,
not stacked on this one), per the one-idea-per-candidate rule.

``side`` is derived from the existing piece encoding (``piece // 6``,
matching this codebase's WP=0..WK=5, BP=6..BK=11 convention already used
throughout fastcore/fastsearch) rather than threaded in as a new parameter,
so no function signature needs an extra argument beyond the reshaped table
itself.
...
def order_moves(moves, count, scores, mail, tt_move, killers, history, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            side_idx = piece // np.uint32(6)
            value = history[side_idx, from_square, to_square]
    ...
    for order_index in range(count):
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                to_square = (move >> np.uint32(6)) & np.uint32(63)
                side_idx = piece // np.uint32(6)
                history[side_idx, from_square, to_square] += depth * depth
            break
        if stop[0]:
            break
