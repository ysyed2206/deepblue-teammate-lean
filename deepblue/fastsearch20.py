"""S1-search20: fastsearch5 (NMP v1) with the null_depth floor fixed.

Priority-list item #6, delta #3 from NMP_DELTA.md, tested in isolation
before any other NMP change per that document's explicit ordering.

fastsearch5's null-move verification computed
``null_depth = max(0, depth - 1 - NMP_REDUCTION)`` and recursed into
``negamax`` at that depth.  With ``NMP_MIN_DEPTH=4`` and
``NMP_REDUCTION=3``, the smallest depth that can trigger NMP at all
(depth=4) produces ``null_depth=0`` -- which routes straight into
quiescence().  So the *most common* trigger depth turned NMP's "cheap
fail-high proof" into a full tactical qsearch, not a shallow probe. This is
NMP_DELTA.md's top candidate explanation for fastsearch5's 40.6% paired
score.

This candidate changes exactly one thing: NMP is only attempted when the
resulting null_depth would already be >= 1 (skip NMP entirely rather than
clamp into qsearch when it wouldn't). Every other guard (non-PV, not in
check, non-pawn-material, no consecutive null, static_eval >= beta,
EP-aware null hash, decisive-score clamp on the return) is unchanged from
fastsearch5. Not combined with the TT-consistency guard or the
verification-search deltas (#1, #2 in NMP_DELTA.md) -- those are separate,
later candidates, per the "one idea per candidate" rule.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # NMP_DELTA.md #3 instrumentation: how often would fastsearch5's old
    # rule have tried NMP right where this fix now refuses (the exact
    # shallow-depth case that used to drop into quiescence)?
    if (
        ply > 0
        and not is_pv
        and not checked
        ...
        and depth - 1 - NMP_REDUCTION < 1
        and abs(beta) < MATE_THRESHOLD
        and st[3] < 99
    ):
        counters[NMP_SKIPPED_SHALLOW] += 1
