...
FITTED KING-ATTACK WEIGHTS (fastsearch173, 2026-09-09). fastsearch140's only
change was raising the queen's king-zone attack weight from 10 to 202, on the
classic attack-unit ratio. It measured FLAT over 120 games -- which at that
sample resolves nothing finer than +/-50 Elo -- and was shipped anyway because
it fixed one blunder position.

Fitted against 120k quiet positions carrying deep Stockfish scores, 202 is the
worst queen weight anywhere in 10..202, and N 62 / B 42 / R 65 / Q 100 gives
validation MSE 0.031663 against 0.032537 for the shipped set.

Two of the shipped weights were never measured at all: fastsearch141 was meant
to test B=81/R=121 but edited the shared constants file, making 140 and 141
byte-identical -- its "-34 Elo" is one build playing itself.

This build is SHIPPED WITHOUT A SELF-PLAY MATCH, at the user's direction; the
evidence for it is the Stockfish fit plus the record above, not a match.
"""
...
from deepblue.eval_terms import (
    game_phase,
    king_attack_danger_tuned_white_relative,
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    extra += king_attack_danger_tuned_white_relative(bb, phase, _TOTAL_PHASE)
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if ply > 0 and not suppress_repetition:
        ...
        # ONE prior occurrence is enough, wherever it came from
        # (fastsearch173, 2026-09-09). This deliberately reverses the rule
        # above it, which scored a repeat of a SEARCH position as a draw but
        # a repeat of a REAL GAME position as nothing. That is backwards: the
        # game-history repeat is the one actually walking toward a threefold.
        #
        # Round 79 is the cost. From a position the engine rated +195 it
        # played Qc8 at move 58 -- only the second occurrence, so nothing here
        # fired -- Black repeated in turn, and the referee claimed the
        # threefold before we ever reached a node this code would have
        # rejected. A full point, from a won game, with no rule broken.
        #
        # Scoring one repetition as -CONTEMPT does NOT force us to avoid
        # draws we need: the line is worth -20, so at +195 we walk away from
        # it, and at -300 we still take it gratefully. It only removes the
        # case where shuffling looks free.
        if prior_total >= 1 or prior_path >= 1:
            return -CONTEMPT
    if insufficient_material(bb, occ):
        return DRAW_SCORE
