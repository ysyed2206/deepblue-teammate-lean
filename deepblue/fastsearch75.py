"""S1-search75: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Captures that lose material go below killers and quiet history, but
# keep their MVV-LVA order relative to each other.
...
@njit(cache=True, nogil=True)
def order_moves(moves, count, scores, mail, tt_move, killers, history, ply, bb, occ, st):
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    order_moves(pseudo[ply], count, scores[ply], mail, tt_move, killers, history, ply, bb, occ, st)
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history, 0, bb, occ, st)
