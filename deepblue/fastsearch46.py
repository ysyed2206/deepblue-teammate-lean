"""S1-search46: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Counter-move heuristic. A quiet move that refuted the opponent's LAST move
# once will often refute it again wherever that same move reappears in the
# tree, which is information neither killers (indexed by ply) nor history
# (indexed by piece/destination alone) capture. Ordered above history but
# below the killers, since a killer proved itself at this exact ply.
ORDER_COUNTER = 1 << 19
...
def order_moves(moves, count, scores, mail, tt_move, killers, history, ply,
                counter_moves, prev_piece, prev_to):
    for index in range(count):
        ...
        elif prev_piece < 12 and move == counter_moves[prev_piece, prev_to]:
            scores[index] = ORDER_COUNTER
        else:
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    prev_piece = (prev_move >> np.uint32(12)) & np.uint32(15)
    prev_to = (prev_move >> np.uint32(6)) & np.uint32(63)
    ...
    for order_index in range(count):
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                # Remember WHICH reply refuted the opponent's last move.
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
                ...
                tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                killers, history, piece_keys, side_key, castle_keys, ep_file_keys,
                root_moves, root_scores, previous_best, counter_moves):
    side = st[0]
    path_hashes[0] = value
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history, 0,
                counter_moves, np.uint32(12), np.uint32(0))
...
class FastEngine46:
    ...
    def __init__(self) -> None:
        ...
        self.counter_moves = np.zeros((12, 64), dtype=np.uint32)
