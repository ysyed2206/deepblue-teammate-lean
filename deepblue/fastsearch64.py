"""S1-search64: fastsearch4 + a transposition table probe/store inside qsearch.
...
# ProbCut (donor: Ethereal's search.c/search.h). See fastsearch62's own
# docstring for the full rationale; constants are Ethereal's real values.
PROBCUT_DEPTH = 5
PROBCUT_MARGIN = 100

# Continuation history / 1-ply counter-move history (donor: Ethereal's
# history.c, its real source). The insight: a quiet move's value often
# depends on what the OPPONENT just played, not just which of our own
# pieces moves where -- e.g. a knight recapture on a square the last
# move just vacated is a very different move than the same knight jump
# in a quiet position. Indexed by (previous move's piece, previous
# move's to-square, this move's piece, this move's to-square) and added
# to the existing butterfly history for ordering, exactly like Ethereal
# sums its own multiple history sources. Deliberately kept to the SAME
# simple additive update this codebase's existing history already uses
# (+= depth*depth, clamped at read time) rather than also adopting
# Ethereal's different 'gravity' update formula -- that would be a
# second, separate idea bundled into one candidate.
...
@njit(cache=True, nogil=True)
def order_moves(moves, count, scores, mail, tt_move, killers, history, continuation_history, prev_piece, prev_square, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            if prev_piece != np.uint32(NO_PIECE):
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, continuation_history, prev_piece, prev_square,
            piece_keys, side_key, castle_keys, ep_file_keys):
    counters[NODES] += 1
    if stop[0]:
        return 0
    ...
    tt_move = np.uint32(0)
    tt_hit = False
    tt_hit_score = np.int64(0)
    tt_hit_depth = np.int16(-1)
    counters[TTPROBE] += 1
    if tt_usable and tt_key[index] == value:
        counters[TTHIT] += 1
        tt_move = tt_move_arr[index]
        tt_hit = True
        tt_hit_score = score_from_tt(tt_score[index], ply)
        tt_hit_depth = tt_depth[index]
    ...
    r_beta = beta + PROBCUT_MARGIN
    mate_ceiling = MATE_SCORE - MAX_PLY - 1
    if r_beta > mate_ceiling:
        r_beta = mate_ceiling
    if (
        not is_pv
        and not checked
        and depth >= PROBCUT_DEPTH
        and abs(beta) < MATE_THRESHOLD
        and (not tt_hit or tt_hit_score >= r_beta or tt_hit_depth < depth - 3)
    ):
        tactical_count = generate_pseudo_tactical(bb, occ, mail, st, pseudo[ply])
        order_qmoves(pseudo[ply], tactical_count, scores[ply])
        for pc_index in range(tactical_count):
            pc_move = pick_best(pseudo[ply], scores[ply], tactical_count, pc_index)
            pc_old_castle = st[1]
            pc_old_ep_file = Z.canonical_ep_file(bb, occ, st)
            make_move(bb, occ, mail, st, pc_move, undo, ply)
            if in_check(bb, occ, st, side):
                unmake_move(bb, occ, mail, st, pc_move, undo, ply)
                continue
            pc_new_ep_file = Z.canonical_ep_file(bb, occ, st)
            pc_child = Z.apply_move(value, np.uint64(pc_move), side, pc_old_castle,
                                    pc_old_ep_file, st[1], pc_new_ep_file,
                                    piece_keys, side_key, castle_keys, ep_file_keys)
            pc_move_piece = (pc_move >> np.uint32(12)) & np.uint32(15)
            pc_move_to_square = (pc_move >> np.uint32(6)) & np.uint32(63)
            pc_value = -INFINITY
            if depth >= 2 * PROBCUT_DEPTH:
                pc_value = -quiescence(bb, occ, mail, st, -r_beta, -r_beta + 1, ply + 1,
                                       pc_child, pseudo, scores, undo, stop, counters, probe,
                                       tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                                       piece_keys, side_key, castle_keys, ep_file_keys)
            if depth < 2 * PROBCUT_DEPTH or pc_value >= r_beta:
                pc_value = -negamax(bb, occ, mail, st, depth - 4, -r_beta, -r_beta + 1,
                                    ply + 1, pc_child, pseudo, scores, undo, stop, counters,
                                    probe, path_hashes, game_hashes, game_count,
                                    tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                                    killers, history, continuation_history,
                                    pc_move_piece, pc_move_to_square,
                                    piece_keys, side_key, castle_keys, ep_file_keys)
            unmake_move(bb, occ, mail, st, pc_move, undo, ply)
            if stop[0]:
                break
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    order_moves(pseudo[ply], count, scores[ply], mail, tt_move, killers, history, continuation_history, prev_piece, prev_square, ply)
    ...
    for order_index in range(count):
        ...
        child = Z.apply_move(value, np.uint64(move), side, old_castle, old_ep_file,
                             st[1], new_ep_file, piece_keys, side_key, castle_keys, ep_file_keys)
        move_piece = (move >> np.uint32(12)) & np.uint32(15)
        move_to_square = (move >> np.uint32(6)) & np.uint32(63)
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             ...
                             tt_move_arr, killers, history, continuation_history,
                             move_piece, move_to_square,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
            score = -negamax(bb, occ, mail, st, child_depth, -alpha - 1, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, continuation_history,
                             move_piece, move_to_square,
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                if killers[ply, 0] != move:
                    ...
                    killers[ply, 0] = move
                history[move_piece, move_to_square] += depth * depth
                if prev_piece != np.uint32(NO_PIECE):
                    continuation_history[prev_piece, prev_square, move_piece, move_to_square] += depth * depth
            break
        if stop[0]:
            break
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
                probe, path_hashes, game_hashes, game_count,
                tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                killers, history, continuation_history, piece_keys, side_key, castle_keys, ep_file_keys,
                root_moves, root_scores, previous_best):
    side = st[0]
    path_hashes[0] = value
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[0])
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history, continuation_history, np.uint32(NO_PIECE), np.uint32(0), 0)
    ...
    for order_index in range(count):
        ...
        child = Z.apply_move(value, np.uint64(move), side, old_castle, old_ep_file,
                             st[1], new_ep_file, piece_keys, side_key, castle_keys, ep_file_keys)
        root_move_piece = (move >> np.uint32(12)) & np.uint32(15)
        root_move_to_square = (move >> np.uint32(6)) & np.uint32(63)
        if legal == 0:
            score = -negamax(bb, occ, mail, st, depth - 1, np.int64(-INFINITY), -alpha,
                             ...
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, continuation_history,
                             root_move_piece, root_move_to_square,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
...
class FastEngine64:
    ...
    def __init__(self) -> None:
        ...
        # (prev piece, prev to-square, piece, to-square) -- persists
        # across moves within a game exactly like self.history above.
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                legal = search_root(
                    ...
                    self.killers, self.history, self.continuation_history,
