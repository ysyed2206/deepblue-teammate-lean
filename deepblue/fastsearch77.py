"""S1-search77: fastsearch4 + a transposition table probe/store inside qsearch.
...
@njit(cache=True, nogil=True)
def order_moves(moves, count, scores, mail, tt_move, killers, history, continuation_history, prev_moves, ply):
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    order_moves(pseudo[ply], count, scores[ply], mail, tt_move, killers, history, continuation_history, prev_moves, ply)
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                if prev_cut != np.uint32(0):
                    prev_cut_piece = (prev_cut >> np.uint32(12)) & np.uint32(15)
                    prev_cut_square = (prev_cut >> np.uint32(6)) & np.uint32(63)
                    if prev_cut_piece < np.uint32(12):
                        continuation_history[prev_cut_piece, prev_cut_square,
                                             piece, to_square] += depth * depth
            break
        if stop[0]:
            break
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history, continuation_history, prev_moves, 0)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.killers, self.history, self.continuation_history, self.prev_moves,
