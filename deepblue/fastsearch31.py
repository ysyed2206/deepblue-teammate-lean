"""S1-search31: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import extra_white_relative
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    """Base PST evaluation plus king safety and passed pawns.

    Signature is identical to the base evaluator so every existing call site
    in this file works unchanged. ``extra_white_relative`` is White-relative,
    so it is flipped for Black exactly the way the base evaluation flips its
    own score.
    """
    ...
    if phase > _TOTAL_PHASE:
        phase = _TOTAL_PHASE
    extra = extra_white_relative(bb, phase, _TOTAL_PHASE)
    if st[0] != _WHITE:
        extra = -extra
    return score + extra
