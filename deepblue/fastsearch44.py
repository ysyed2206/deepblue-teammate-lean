"""S1-search44: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import rook_files_white_relative
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    """Base PST evaluation plus rook open-file/seventh-rank bonuses.

    Isolated candidate, per this project's one-idea-per-test rule -- see
    eval_terms.py's docstring for why (a bundled candidate cost 25 points of
    measured Elo purely by making attribution impossible). Directly evidenced
    by round 10's loss (moves 27-36): White's doubled rooks dominated the
    e-file while Yumo's evaluation had no concept of file control at all.
    """
