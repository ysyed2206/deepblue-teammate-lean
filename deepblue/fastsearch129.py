"""S1-search129: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import (
    game_phase,
    king_attack_danger_v2_white_relative,
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # v2: pressure on an EXTENDED king zone, not presence on a 9-square ring.
    # Round 60 was drawn from a position this engine scored at -404 (winning by
    # four pawns) while being mated at; the old term read 11cp through the whole
    # attack because only the knight touched the ring -- queen, bishops and both
    # rooks scored zero while one rank away. Decomposition of the actual moves
    # (tools/explain_eval.py) showed each queen grab paying +96cp of material
    # against 11cp of danger, which is why the queen kept eating.
    extra += king_attack_danger_v2_white_relative(bb, phase, _TOTAL_PHASE)
