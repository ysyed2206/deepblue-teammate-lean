"""S1-search124: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Sentinel for "no static evaluation was computed at this ply". Real
# evaluations are bounded well inside +/-INFINITY (31000), so -32000 cannot
# collide with one.
EVAL_UNKNOWN = np.int64(-32000)
# When the side to move is NOT improving, pruning margins shrink to this
# fraction -- deteriorating positions get pruned harder. Deliberately a mild
# 3/4 rather than the aggressive depth-1 form used by engines that search
# twice as deep as this one.
NOT_IMPROVING_NUM = 3
NOT_IMPROVING_DEN = 4

# Multi-cut. Only at depths where a 4-ply reduction still leaves a search
# worth trusting, and requiring 3 of the first 6 legal moves to fail high --
# a single fail-high is far too weak a signal to skip a whole subtree.
        ...
        if move == tt_move:
            ...
            if ply >= 1:
                ...
                if prev2 != np.uint32(0):
                    prev2_piece = (prev2 >> np.uint32(12)) & np.uint32(15)
                    prev2_square = (prev2 >> np.uint32(6)) & np.uint32(63)
                    if prev2_piece < np.uint32(12):
                        value += continuation_history2[prev2_piece, prev2_square,
                                                       piece, to_square]
...
def rfp_margin(bb, st, ply, is_pv, checked, depth, tt_move, original_alpha, beta,
                counters, mg_table, eg_table, phase_table, static_eval, improving):
    ...
    if (
        ...
        if not improving:
            margin = (margin * NOT_IMPROVING_NUM) // NOT_IMPROVING_DEN
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, continuation_history, continuation_history2, prev_moves, static_evals,
            piece_keys, side_key, castle_keys, ep_file_keys,
    ...
    # Static evaluation for this node, computed once and shared by reverse
    # futility, razoring, null-move and futility below. Only non-PV,
    # non-check nodes need it -- those are exactly the nodes where any of the
    # four can fire, so this never evaluates a position that previously went
    # unevaluated.
    node_eval = EVAL_UNKNOWN
    if not checked and not is_pv:
        counters[EVAL] += 1
        node_eval = np.int64(evaluate(bb, st, MG_TABLE, EG_TABLE, PHASE_TABLE))
    static_evals[ply] = node_eval
    # Is the side to move better off than on its own previous turn? Unknown
    # counts as improving, the direction that prunes less.
    improving = True
    if ply >= 2 and node_eval != EVAL_UNKNOWN and static_evals[ply - 2] != EVAL_UNKNOWN:
        improving = node_eval > static_evals[ply - 2]
    ...
    # depth, treat the node as a proven fail-high rather than paying for the
    # full-depth search. Gated exactly like null-move pruning -- never at PV
    # or check nodes, never twice in a row -- because it makes the same kind
    # of claim: that this node cannot be the one that matters.
    #
    # pick_best sorts the chosen move into place as it goes, so the prefix it
    # leaves behind is already ordered; the main loop below re-picking from
    # index 0 returns the same moves in the same order.
    if (
        ply > 0
        and not is_pv
        and not checked
        ...
        if mc_reduced > 0:
            ...
            for mc_index in range(count):
                ...
                mc_old_castle = st[1]
                mc_old_ep_file = Z.canonical_ep_file(bb, occ, st)
                counters[MAKE] += 1
                ...
                if in_check(bb, occ, st, side):
                    ...
                    continue
                mc_tried += 1
                mc_new_ep_file = Z.canonical_ep_file(bb, occ, st)
                mc_child = Z.apply_move(value, np.uint64(mc_move), side, mc_old_castle,
                                        mc_old_ep_file, st[1], mc_new_ep_file,
                                        piece_keys, side_key, castle_keys, ep_file_keys)
                ...
                mc_score = -negamax(
                    ...
                    continuation_history, continuation_history2, prev_moves, static_evals,
                    piece_keys, side_key, castle_keys, ep_file_keys,
                    False, True)
                unmake_move(bb, occ, mail, st, mc_move, undo, ply)
                if stop[0]:
                ...
                if mc_score >= beta:
                    ...
                    if mc_cuts >= MULTICUT_CUTOFFS:
                        # A mate score proved by a reduced line is not a real
                        # mate, and beta is the honest lower bound either way.
                        return beta
    ...
    if futility_active:
        futility_eval = node_eval
        futility_margin = FUTILITY_MARGIN_PER_DEPTH * depth
        if not improving:
            futility_margin = (futility_margin * NOT_IMPROVING_NUM) // NOT_IMPROVING_DEN
        futility_eval += futility_margin
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, continuation_history, continuation_history2, prev_moves, static_evals,
                             piece_keys, side_key, castle_keys, ep_file_keys,
                             False, True)
        else:
            reduction = 0
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                for tried in range(order_index):
                    ...
                    if (quiet_captured == np.uint32(NO_PIECE)
                        ...
                        if cont2_piece < np.uint32(12):
                            d_pen = continuation_history2[cont2_piece, cont2_square,
                                                          quiet_piece, quiet_to]
                            continuation_history2[cont2_piece, cont2_square,
                                                  quiet_piece, quiet_to] = (
                                d_pen - bonus - (d_pen * bonus) // HISTORY_MAX
                            )
            break
        if stop[0]:
            break
...
class FastEngine124:
    ...
    def __init__(self) -> None:
        ...
        # second level: (own previous piece, its to-square, this piece, to-square)
        self.continuation_history2 = np.zeros((12, 64, 12, 64), dtype=np.int32)
        self.prev_moves = np.zeros(MAX_PLY + 2, dtype=np.uint32)
        # shared static evaluation per ply; EVAL_UNKNOWN where none was needed
        self.static_evals = np.full(MAX_PLY + 2, EVAL_UNKNOWN, dtype=np.int64)
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        self.killers[:] = 0
        self.prev_moves[:] = 0
        self.static_evals[:] = EVAL_UNKNOWN
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.prev_moves, self.static_evals,
