"""S1-search32: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import game_phase, passed_pawns_white_relative
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    """Base PST evaluation plus passed pawns (isolated candidate).

    Signature matches the base evaluator exactly, so every existing call site
    in this file works unchanged. The extra term is White-relative and is
    flipped for Black the same way the base evaluation flips its own score.
    """
