"""S1-search21: fastsearch18 + SEE bad-noisy skip in qsearch ("Design B").

Priority-list item #3, Design B half (Design A -- SEE-based ordering -- is
a separate, independent candidate; the two are deliberately not stacked,
per this project's one-idea-per-candidate rule and per Agent C's design
sketch in CLAUDE_REMOTE_RESULTS.md / DONOR_NEXT_IDEAS.md).

Donor evidence: Pawnstar's public SPRT is +38.65 +/-13.78 Elo for a sorted
SEE early-exit in qsearch -- but that number bundles the SEE skip WITH
removing qsearch's TT entirely, and Pawnstar has no qsearch TT to begin
with. Deep Blue just added one (fastsearch18). The two ideas are not
assumed additive here; this candidate tests the skip in isolation, on top
of the already-landed qsearch TT, to get Deep Blue's own evidence rather
than transferring Pawnstar's bundled number.

Exact scope, one insertion point only:

- In quiescence()'s NOT-in-check branch main move loop only: a pseudo
  capture (never a promotion -- deepblue/see.py's own docstring says SEE
  must not be used for pruning promotions) with ``see_value < 0`` is
  skipped entirely (never made, never counted toward legal_tactical, never
  searched).
- The checked/evasion branch (a separate loop, lines ~249-282) is
  completely untouched -- skipping a bad-SEE check evasion could mean
  missing the only legal reply and misreporting a false mate. This is a
  hard, structural separation (two different loops), not a runtime flag,
  so there is no condition to get wrong.
- The skip sits AFTER the stand-pat beta cutoff (which already returned
  before this loop is even reached) and its own legality probe
  (has_legal_tactical) is unchanged -- that probe's job is proving
  "not stalemate" and is unaffected by SEE quality.
- A skipped move never increments legal_tactical, so if every tactical
  candidate happens to be skipped, the existing
  ``legal_tactical == 0 -> any_legal_move(...) -> DRAW_SCORE`` fallback
  fires exactly as it already does today when the tactical list is
  legitimately empty -- no new code path, converges to already-tested
  behaviour.
- Accepted, SPRT-measured risk (not a bug): SEE only evaluates the
  immediate exchange on one square, so a move that looks bad now but sets
  up a real follow-on threat can be pruned away. This is exactly the
  trade-off Pawnstar's own SPRT accepted; the point of this candidate is
  to measure that trade-off on Deep Blue's own evaluator, not to assume it
  transfers.

No aspiration windows, null move, LMR, futility or IIR are present
(inherited unchanged from fastsearch4/fastsearch18).
"""
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for index in range(count):
        ...
        # Design B: bad-noisy skip. Non-check branch ONLY -- the checked
        # branch above is a structurally separate loop and is never
        # touched by this, so a bad-SEE check evasion can never be
        # skipped here. Promotions are excluded (see.py's own docstring:
        # not validated for pruning promotions).
        move_captured = (move >> np.uint32(16)) & np.uint32(15)
        move_promotion = (move >> np.uint32(20)) & np.uint32(15)
        if move_captured != np.uint32(NO_PIECE) and move_promotion == np.uint32(NO_PIECE):
            if see_value(bb, occ, mail, st, move) < 0:
                counters[QSEE_SKIP] += 1
                continue
