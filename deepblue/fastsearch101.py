"""S1-search101: fastsearch4 + a transposition table probe/store inside qsearch.
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # Rook placement: open file (+22), semi-open file (+11), seventh rank
    # (+18). rook_files_white_relative has existed in eval_terms.py for the
    # whole project, validated, and no shipped engine has ever called it --
    # the same situation mobility_white_relative was in before it was wired
    # in and measured +61 Elo. Until now the evaluation had no concept of
    # rook activity: a rook buried behind its own pawns and a rook raking an
    # open file scored identically. Round 10 (lost to Triggerfish) was decided
    # by exactly that, White doubling rooks on the e-file over moves 27-36.
