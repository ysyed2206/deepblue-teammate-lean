"""S1-search70: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late move pruning: Ethereal's conservative counts, indexed by depth.
# Aspiration windows: narrow band around the previous iteration's score.
# Razoring: Stockfish's real margin shape, 512 + 293*depth^2.
RAZOR_MAX_DEPTH = 3
RAZOR_BASE = 512
RAZOR_PER_DEPTH = 293
...
LMP_MAX_DEPTH = 8
LMP_COUNTS = np.array([0, 2, 3, 4, 6, 8, 13, 18, 23], dtype=np.int32)
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # Razoring. Only abandons the node if quiescence independently confirms
    # the static verdict, so a tactically alive position is not discarded on
    # a static score alone.
    if (
        not is_pv
        and not checked
        ...
        razor_margin = RAZOR_BASE + RAZOR_PER_DEPTH * depth * depth
        if razor_eval + razor_margin < alpha:
            razor_score = quiescence(bb, occ, mail, st, alpha, beta, ply, value,
