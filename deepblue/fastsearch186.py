...
fastsearch186 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.
...
fastsearch186 (2026-09-09) = fastsearch180 + BUDGET-NEUTRAL stability timing.
...
fastsearch186 (2026-09-09) = fastsearch180 + stability timing + the ply-1
...
fastsearch186 (2026-09-10) = fastsearch185 with AMNESIA: every piece of state
carried between moves is wiped at the start of each search.

WHY THIS EXISTS. Four consecutive games (rounds 90, 94, 97, 98) contain moves
that a FRESH engine, given the same position and the same time, does not play.
Measured directly on round 79, an engine that has played the game agrees with a
fresh one on only 52% of moves; clearing any single component -- the
transposition table, the quiet-move history and killers, the continuation
history, or the correction history -- recovers roughly a quarter of the gap
each, so the effect is the sum of all four rather than one broken part.

That has been called a "divergence problem" throughout this project without
anyone establishing it is a problem at all. Carried state is supposed to HELP:
table reuse and history heuristics are why engines are strong, and a different
move is not automatically a worse one. The only evidence that it costs anything
used this engine as its own referee, which is exactly the blind referee the
Stockfish tooling was built to replace.

This build settles it. If amnesia BEATS 185, the carried state is actively
harmful and there is a large fix available. If amnesia LOSES, divergence is the
sound of the heuristics working and should stop being described as a defect.
Either answer is worth one match; neither has ever been measured.

Note what is NOT lost: the table is still filled and used WITHIN each search,
and the game-history hashes for repetition detection are untouched. Only
CROSS-MOVE reuse is given up.
"""
...
class FastEngine186:
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        self.excluded[:] = 0
        stable_for = 0
        # AMNESIA (see the module docstring): wipe everything carried from the
        # previous move. Deliberately the whole set, because clearing any one
        # of them recovers only about a quarter of the divergence.
        self.tt_key[:] = 0
        self.tt_score[:] = 0
        self.tt_depth[:] = -1
        self.tt_bound[:] = 0
