"""S1-search62: fastsearch4 + a transposition table probe/store inside qsearch.
...
# ProbCut (donor: Ethereal's search.c/search.h, its real source). If a
# strong tactical move already beats a raised beta (beta + margin) at a
# reduced depth, the real move at full depth almost always beats the real
# beta too -- so try the tactical moves first, cheaply, before doing the
# full move loop. Constants are Ethereal's own literal values.
PROBCUT_DEPTH = 5
PROBCUT_MARGIN = 100
    ...
    if (
        ...
        for pc_index in range(tactical_count):
            ...
            if stop[0]:
                break
            if pc_value >= r_beta:
                # Matches this codebase's existing "shortcuts don't write the
                # TT" convention (RFP above never stores either) -- kept
                # simple rather than adding a second store path to audit.
                return pc_value
