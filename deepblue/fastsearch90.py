"""S1-search86: fastsearch4 + a transposition table probe/store inside qsearch.
...
ORDER_CAPTURE = 1 << 21
ORDER_GOOD_CAPTURE = 1 << 21
ORDER_BAD_CAPTURE = 1 << 18
ORDER_KILLER_1 = 1 << 20
...
@njit(cache=True, nogil=True)
def order_moves(bb, occ, mail, st, moves, count, scores, tt_move, killers, history, ply):
    """Stage moves using ideas common to strong modern engines.

    Captures are separated into SEE-good and SEE-bad groups rather than
    assuming MVV-LVA is always safe.  Quiet moves retain TT/killer/history
    ordering.  This is an ordering heuristic only: SEE never prunes here.
    """
    for index in range(count):
        ...
        if move == tt_move:
            ...
            see = see_value(bb, occ, mail, st, move)
            if see >= 0:
                scores[index] = ORDER_GOOD_CAPTURE + see
            else:
                scores[index] = ORDER_BAD_CAPTURE + see
...
def order_qmoves(bb, occ, mail, st, moves, count, scores):
    """SEE-first tactical ordering: winning exchanges before speculative ones."""
    for index in range(count):
        move = moves[index]
        piece = (move >> np.uint32(12)) & np.uint32(15)
        captured = (move >> np.uint32(16)) & np.uint32(15)
        ...
        if captured != np.uint32(NO_PIECE):
            see = see_value(bb, occ, mail, st, move)
            if see >= 0:
                score += ORDER_GOOD_CAPTURE + see
            else:
                score += ORDER_BAD_CAPTURE + see
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for index in range(count):
        ...
        # Learn from quiet moves in both directions. Strong engines use
        # history as a statistical ordering signal, not just a success counter.
        if move_captured0 == np.uint32(NO_PIECE) and move_promotion0 == np.uint32(NO_PIECE):
            hpiece = (move >> np.uint32(12)) & np.uint32(15)
            hto = (move >> np.uint32(6)) & np.uint32(63)
            if score > alpha_before_move:
                update_history(history, hpiece, hto, depth * depth * 4)
            else:
                update_history(history, hpiece, hto, -(depth * depth))
...
def update_history(history, piece, to_square, bonus):
    """Gravity-style history update used by modern engines.

    Unlike an unbounded += depth^2 table, this learns both good and bad quiet
    moves while naturally forgetting old evidence as entries approach their
    limits.
    """
    old = history[piece, to_square]
    if bonus > HISTORY_CEILING:
        bonus = HISTORY_CEILING
    elif bonus < HISTORY_FLOOR:
        bonus = HISTORY_FLOOR
    value = old + bonus - (old * abs(bonus)) // HISTORY_CEILING
    if value > HISTORY_CEILING:
        value = HISTORY_CEILING
    elif value < HISTORY_FLOOR:
        value = HISTORY_FLOOR
    history[piece, to_square] = value
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # Razoring: if the static position is already far below alpha at a shallow
    # non-PV node, a full-width search is unlikely to change the result.  Verify
    # with quiescence rather than blindly pruning. This mirrors a common modern
    # search pattern while keeping the cutoff deliberately conservative.
    if (
        ply > 0
        and not is_pv
        and not checked
        ...
        razor_margin = 180 + 120 * depth
        if razor_eval + razor_margin <= alpha:
            razor = quiescence(
                bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop,
                counters, probe, tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                piece_keys, side_key, castle_keys, ep_file_keys
            )
            if razor <= alpha:
    ...
    count = generate_pseudo_legal(bb, occ, mail, st, pseudo[ply])
    counters[ORDER] += 1
    order_moves(bb, occ, mail, st, pseudo[ply], count, scores[ply], tt_move, killers, history, ply)
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(bb, occ, mail, st, pseudo[0], count, scores[0], previous_best, killers, history, 0)
