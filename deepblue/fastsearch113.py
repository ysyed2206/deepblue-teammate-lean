"""S1-search113: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import (
    ...
    king_safety_tuned_white_relative,
    mobility_tuned_white_relative,
    passed_pawns_tuned_white_relative,
    bishop_pair_tuned_white_relative,
    pawn_structure_tuned_white_relative,
)
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    extra = passed_pawns_tuned_white_relative(bb)
    extra += king_safety_tuned_white_relative(bb, phase, _TOTAL_PHASE)
    extra += king_attack_danger_white_relative(bb, phase, _TOTAL_PHASE)
    extra += mobility_tuned_white_relative(bb)
    ...
    extra += bishop_pair_tuned_white_relative(bb)
    ...
    extra += pawn_structure_tuned_white_relative(bb)
    if st[0] != _WHITE:
        extra = -extra
    return score + extra
