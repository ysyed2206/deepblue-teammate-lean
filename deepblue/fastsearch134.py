"""S1-search134: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Transposition table size. 22 bits is 4M entries = 76 MB, against a 2048 MB
# competition limit -- we were using 3.7% of available memory. Measured TT miss
# rate is 72.8%, and quiescence (48.7% of all nodes) shares this same table, so
# over a 40-move game tens of millions of positions are stored into 4M slots
# and the table is overwritten many times over.
#
# 24 bits is 16M entries = 304 MB, still under 15% of the limit. Published
# figures put the first hash doubling at +50-70 Elo with sharply diminishing
# returns after, and warn that an oversized table costs more in TLB misses
# than it gains -- which is why this stops at 24 rather than going to 25
# (608 MB) and why it is measured rather than assumed.
TT_BITS = 24
TT_SIZE = 1 << TT_BITS
