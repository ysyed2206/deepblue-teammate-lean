"""S1-search146: fastsearch4 + a transposition table probe/store inside qsearch.
...
# How much of the soft budget must be spent before the predictive stop may end
# the search.
#
# Round 72 (2026-09-08), a 71-move loss with NO move flagged as an error by
# Stockfish or by our own scan. The evaluation bled from -193 at move 36 to
# -893 at move 57 in slices of 20-47cp, while the knight shuffled between the
# same three squares. Per-move clock in that game:
#
#     moves <= 25 :  3.57 s/move   (18 real thinking moves)
#     moves >= 40 :  1.62 s/move   (23 real thinking moves)
#
# The allocator was NOT the constraint. At move 40 with ~30s left it budgeted
# ~2.9s and the search used 1.62s -- 56% of it -- while in the opening the
# search ran OVER soft into hard time. Endgame iterations are cheap and deep,
# so the growth prediction fires early and quits: we spend most where the
# position was handed to us, least where the game is actually lost.
#
# An earlier version of this change (fastsearch128) was shelved when a broader
# claim about wasted clock collapsed under measurement. That claim was wrong.
# This is the narrow, evidenced version of it.
#
# The downside is bounded and known: an aborted iteration is discarded, so the
# cost is wasted work and never a lost move -- the hard timer still ends the
# search and the last completed iteration's move stands.
