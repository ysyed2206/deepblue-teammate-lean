"""S1-search87: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Stability-aware time management (fastsearch87). Tolerances are centipawns
# / iteration counts, not milliseconds -- the actual time effect follows from
# how many iterations that takes at the position's real branching factor.
STABLE_SCORE_TOLERANCE_CP = 15
EARLY_STOP_STABLE_ITERATIONS = 3
EARLY_STOP_MIN_DEPTH = 8
EARLY_STOP_MIN_ELAPSED_FRACTION = 0.15
INSTABILITY_SOFT_EXTENSION = 1.6
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                if legal == 0 or self.stop[0]:
                    break
                new_move = int(self.root_moves[0])
                new_score = int(self.root_scores[0])
                if (completed > 0 and new_move == best_move
                        and abs(new_score - best_score) <= STABLE_SCORE_TOLERANCE_CP):
                    stable_iterations += 1
                else:
                    stable_iterations = 0
                best_move = new_move
                ...
                if stable_iterations == 0:
                    effective_soft_ms = min(hard_ms * 0.9, soft_ms * INSTABILITY_SOFT_EXTENSION)
                if elapsed >= effective_soft_ms:
                    break
                if abs(best_score) > MATE_THRESHOLD and depth >= MATE_SCORE_MIN_DEPTH:
                    break
                if (stable_iterations >= EARLY_STOP_STABLE_ITERATIONS
                        and depth >= EARLY_STOP_MIN_DEPTH
                        and elapsed >= soft_ms * EARLY_STOP_MIN_ELAPSED_FRACTION):
                    break
                if previous_ms > 1.0:
