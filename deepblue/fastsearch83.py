"""S1-search83: fastsearch4 + a transposition table probe/store inside qsearch.
...
# "Easy move" / stability early exit. If the same root move survives several
# consecutive deepenings and the score has not fallen, more thinking almost
# never changes the answer, so return it and bank the clock.
#
# This matters in two separate places. In the daily puzzle challenge the
# scoring rewards answering FAST as well as correctly, and without this the
# engine spends its whole allocation even on an obvious move. In real games
# the analysis of rounds 31-35 showed the engine reaching the late middlegame
# with only ~0.7 s/move left; time saved on obvious early moves is time
# available exactly where games are actually decided.
#
# The guards are what keep it honest: a minimum depth so it cannot fire on a
# shallow accident, several consecutive agreeing iterations rather than one,
# and a refusal to bail when the score is DROPPING -- a falling score means
# the position is turning against us, which is when extra thought is most
# valuable, not least.
# HARD GATE: the early exit is allowed ONLY when the per-move budget is far
# larger than any real game can produce. Measured with the shipped
# time_manager: a 120 s game allocates 2.0-4.7 s per move at every stage,
# while the 20-minute / 5-position puzzle round allocates 43.2 s. A 10 s
# threshold therefore cannot be reached in a rated game and is always
# exceeded in the puzzle round, so this feature is structurally incapable of
# changing how we play a real game -- it is not a matter of tuning.
EASY_MOVE_MIN_BUDGET_MS = 10_000.0

EASY_MOVE_STABLE = 6
EASY_MOVE_MIN_DEPTH = 11
EASY_MOVE_MAX_DROP = 20
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                if best_move == previous_root_move:
                    stable_count += 1
                else:
                    stable_count = 0
                score_drop = previous_root_score - best_score
                previous_root_move = best_move
                previous_root_score = best_score
                completed = depth
                ...
                if abs(best_score) > MATE_THRESHOLD and depth >= MATE_SCORE_MIN_DEPTH:
                    break
                if (
                    soft_ms > EASY_MOVE_MIN_BUDGET_MS
                    and stable_count >= EASY_MOVE_STABLE
                    and depth >= EASY_MOVE_MIN_DEPTH
                    and score_drop <= EASY_MOVE_MAX_DROP
                ):
                    break
                if previous_ms > 1.0:
