"""S1-search160: fastsearch4 + a transposition table probe/store inside qsearch.
...
def order_moves(moves, count, scores, mail, tt_move, killers, history,
                continuation_history, continuation_history2, prev_moves, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # The side's OWN previous move, two plies back, in its own table.
            if ply >= 1:
                prev2 = prev_moves[ply - 1]
                if prev2 != np.uint32(0):
                    p2 = (prev2 >> np.uint32(12)) & np.uint32(15)
                    s2 = (prev2 >> np.uint32(6)) & np.uint32(63)
                    if p2 < np.uint32(12):
                        value += continuation_history2[p2, s2, piece, to_square]
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, continuation_history, continuation_history2, prev_moves, corr_hist,
            piece_keys, side_key, castle_keys, ep_file_keys,
        ...
        if mc_reduced > 0:
            ...
            for mc_index in range(count):
                ...
                mc_score = -negamax(
                    ...
                    continuation_history, continuation_history2, prev_moves,
                    corr_hist, piece_keys, side_key, castle_keys, ep_file_keys,
                    False, True)
                unmake_move(bb, occ, mail, st, mc_move, undo, ply)
                if stop[0]:
                    break
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, continuation_history, continuation_history2, prev_moves, corr_hist,
                             piece_keys, side_key, castle_keys, ep_file_keys,
                             False, True)
        else:
            reduction = 0
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                cont2_piece = np.uint32(12)
                cont2_square = np.uint32(0)
                if ply >= 1:
                    prev_cut2 = prev_moves[ply - 1]
                    if prev_cut2 != np.uint32(0):
                        cont2_piece = (prev_cut2 >> np.uint32(12)) & np.uint32(15)
                        cont2_square = (prev_cut2 >> np.uint32(6)) & np.uint32(63)
                if cont2_piece < np.uint32(12):
                    d_entry = continuation_history2[cont2_piece, cont2_square, piece, to_square]
                    continuation_history2[cont2_piece, cont2_square, piece, to_square] = (
                        d_entry + bonus - (d_entry * bonus) // HISTORY_MAX
                    )
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history,
                continuation_history, continuation_history2, prev_moves, 0)
...
class FastEngine160:
    ...
    def __init__(self) -> None:
        ...
        # second level: the side's own previous move, two plies back
        self.continuation_history2 = np.zeros((12, 64, 12, 64), dtype=np.int32)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.continuation_history, self.continuation_history2,
                        self.prev_moves, self.corr_hist,
