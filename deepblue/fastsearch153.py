"""S1-search153: fastsearch4 + a transposition table probe/store inside qsearch.
...
from deepblue.eval_terms import (
    ...
    passed_pawns_steep_white_relative,
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    # 143cp on the seventh rather than 95. Round 73 traded a pawn one
    # square from queening for an ordinary pawn.
    extra = passed_pawns_steep_white_relative(bb)
...
# Contempt on TERMINAL draws too. Round 71 traded an exchange-up position
# into king+bishop vs king; DRAW_SCORE 0 scored that as neutral rather
# than as the failure it was.
DRAW_CONTEMPT = np.int64(20)
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history, continuation_history, prev_moves, corr_hist,
                             piece_keys, side_key, castle_keys, ep_file_keys,
                             False, True)
        else:
            reduction = 0
    ...
    if legal == 0:
        return -MATE_SCORE + ply if checked else -DRAW_CONTEMPT
    if st[3] >= 100:
        return -DRAW_CONTEMPT
