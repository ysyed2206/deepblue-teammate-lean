"""S1-search66: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late move reductions. Ethereal's real coefficients; table built at
# import because the hot path cannot afford log() per move.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVES = 4
_LMR_MAX = 64
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            if reduction > 0 and score > alpha and not stop[0]:
                # The reduced probe beat alpha, so the reduction was wrong for
                # this move: verify it at full depth before trusting it.
