"""S1-search130: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Late Move Pruning. Once enough moves have been tried at a shallow node and
# none has raised alpha, the ordering has already had its best chances and the
# remaining quiets are overwhelmingly unlikely to be the move. Skip them
# outright rather than searching each to depth-1.
#
# WHY THIS IS BEING RETRIED. tools/sprt_gate.py's own docstring records that
# LMP was rejected in this project at 60 ms/move. At 60ms this engine reaches
# depth 4, and LMP is depth-gated -- it could not fire in the test that
# rejected it. Three other techniques rejected in that same batch (null move,
# LMR, razoring) were later retested at realistic time controls and all three
# shipped. LMP is the one that never got its retest.
#
# The counts are deliberately generous (a quiet is only pruned once 6+ moves
# have failed at depth 1, 12+ at depth 2, 20+ at depth 3): a cheap technique
# that prunes too eagerly loses real moves, and the measured wins in this
# engine have all come from conservative gates.
LMP_MAX_DEPTH = 3
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if (
            ...
            continue
        # Late move pruning. Same guards as futility (non-PV, not in check,
        # shallow, quiet, not the TT move) plus a move count that has to be
        # exceeded before anything is discarded.
        if (
            not is_pv
            and not checked
