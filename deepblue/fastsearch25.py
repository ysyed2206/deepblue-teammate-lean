"""S1-search25: fastsearch18 + symmetric history malus.

History decomposition candidate #2 of 3 (priority #2). Parent: fastsearch18
(qsearch TT), independent of candidate #1 (fastsearch24, butterfly
indexing) -- built on fastsearch18's ORIGINAL piece-to (12,64) table, not
stacked on 24's reindex, per the one-idea-per-candidate rule. Attribution
to fastsearch18's own qsearch-TT changes is preserved verbatim below.

Donor research (DONOR_NEXT_IDEAS.md "History decomposition") found a
symmetric (exactly ``-bonus``) malus is what Pawnstar and Ethereal use, and
that no donor uses the rejected internal Patch A's flat 5x asymmetric
malus. This candidate tests the simplest, smallest slice: every quiet move
TRIED at a node before the eventual cutoff gets malus``-depth*depth``
(the exact negation of the existing bonus formula) when that node
resolves with a cutoff on a DIFFERENT quiet move. No decoupling of
malus/bonus scale, no gravity/clamping, no width-damping -- those are
separate, later candidates per the decomposition list. A node that never
cuts (fails low / all-node) gets no history update at all, matching
current behaviour (bonus already only applies on a cutoff).

Mechanism: a new per-ply scratch buffer (``tried_quiets``, preallocated in
__init__ like the existing ``pseudo``/``scores`` per-ply arrays -- no
per-node heap allocation in the hot path) records every quiet move
actually made and searched at a node, in order. When a cutoff occurs on a
quiet move, the existing bonus still applies to that move; malus is then
applied to every OTHER quiet move already recorded in the buffer for this
node (i.e. every quiet tried and rejected before the one that cut).
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, tried_quiets, piece_keys, side_key, castle_keys, ep_file_keys):
    counters[NODES] += 1
    if stop[0]:
        return 0
    ...
    for order_index in range(count):
        ...
        if move_captured == np.uint32(NO_PIECE) and move_promotion == np.uint32(NO_PIECE):
            tried_quiets[ply, tried_count] = move
            tried_count += 1
        new_ep_file = Z.canonical_ep_file(bb, occ, st)
        child = Z.apply_move(value, np.uint64(move), side, old_castle, old_ep_file,
                             st[1], new_ep_file, piece_keys, side_key, castle_keys, ep_file_keys)
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, depth - 1, -beta, -alpha, ply + 1, child,
                             ...
                             tt_move_arr, killers, history, tried_quiets,
                             piece_keys, side_key, castle_keys, ep_file_keys)
        else:
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                # Symmetric malus: every OTHER quiet already tried at this
                # node (i.e. every entry except the cutting move itself,
                # which was just appended above and is therefore last).
                for qi in range(tried_count - 1):
                    qmove = tried_quiets[ply, qi]
                    qpiece = (qmove >> np.uint32(12)) & np.uint32(15)
                    qto = (qmove >> np.uint32(6)) & np.uint32(63)
                    history[qpiece, qto] -= depth * depth
            break
        if stop[0]:
            break
...
class FastEngine25:
    ...
    def __init__(self) -> None:
        ...
        # Scratch: quiet moves tried (made and searched) at the current node,
        # per ply -- preallocated like pseudo/scores, no per-node heap
        # allocation in the hot path. Used only for the symmetric malus.
        self.tried_quiets = np.zeros((MAX_PLY, MAX_MOVES), dtype=np.uint32)
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                legal = search_root(
                    ...
                    self.killers, self.history, self.tried_quiets,
