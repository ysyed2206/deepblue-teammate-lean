"""S1-search48: fastsearch4 + a transposition table probe/store inside qsearch.
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    """Base PST evaluation plus passed pawns and king safety, on top of the
    null-move search.

    Stacks the two strongest separately-measured results: null-move pruning
    (fastsearch34, 59.4%) and the two eval terms together (fastsearch35,
    69.8%), each over 96 games against fastsearch18. Additivity is NOT
    assumed -- fastsearch31 showed two positive-in-isolation terms combining
    to 44.3% -- so this stack carries its own evidence.
    """
...
# --- Forward pruning stack -------------------------------------------------
# Pure alpha-beta with perfect move ordering bottoms out around sqrt(35) ~= 5.9
# effective branching factor for chess. This engine already measures ~5.3, so
# better move ordering cannot buy meaningful depth -- SEE ordering (49.0%) and
# aspiration windows (46.8%) both measured neutral for exactly that reason.
# Getting BELOW the alpha-beta floor requires forward pruning: deliberately
# not searching moves a sound search would. That is where depth 12-15 comes
# from, and it is the only route to it.
#
# LMR: search late quiet moves shallower, re-search at full depth if one
# unexpectedly beats alpha. Reduction grows with both depth and move number,
# logarithmically -- a flat reduction over-prunes shallow nodes and
# under-prunes deep ones.
LMR_MIN_DEPTH = 3
LMR_MIN_MOVE = 2

# LMP: skip late quiet moves outright at shallow depth. The previous attempt
# (fastsearch28) measured 6.2% -- catastrophic -- because it had no is_pv
# guard and so blind-pruned inside the principal variation, where the exact
# score decides the move actually played. That guard is the fix.
LMP_MAX_DEPTH = 6
LMP_BASE = 3
...
def _build_lmr_table():
    ...
    for d in range(1, 64):
        for m in range(1, 64):
            table[d, m] = int(0.75 + math.log(d) * math.log(m) / 2.25)
    return table
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            ...
            # Documented gap closed: neither LMP nor LMR previously exempted
            # a quiet move that GIVES check (only "not checked", meaning not
            # currently evading one). A checking move is forcing in exactly
            # the way a capture or promotion is, and pruning/reducing it
            # risks missing forced tactics -- this is DONOR_NEXT_IDEAS.md's
            # documented, never-built "LMP-3 gives-check carve-out", closed
            # here for both LMP and LMR together since both share the same
            # underlying risk.
            gives_check = in_check(bb, occ, st, 1 - side)
            is_quiet = (move_captured == np.uint32(NO_PIECE)
                        and move_promotion == np.uint32(NO_PIECE)
                        and not gives_check)

            # Late move pruning: past a movecount threshold that grows with
            # depth, quiet moves are skipped outright. NEVER inside the PV.
            if (
                is_quiet
                and not is_pv
                and not checked
                and depth <= LMP_MAX_DEPTH
                and legal >= LMP_BASE + depth * depth
                and move != tt_move
            ...
            if (
                ...
                and legal > LMR_MIN_MOVE
                and not checked
                and move != tt_move
            ):
                d_index = depth if depth < 64 else 63
                m_index = legal if legal < 64 else 63
                reduction = LMR_TABLE[d_index, m_index]
            ...
            score = -negamax(bb, occ, mail, st, depth - 1 - reduction, -alpha - 1, -alpha,
                             ply + 1, child,
                             pseudo, scores, undo, stop, counters, probe, path_hashes,
                             game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                             tt_move_arr, killers, history,
            ...
            if reduction > 0 and score > alpha and not stop[0]:
                score = -negamax(bb, occ, mail, st, depth - 1, -alpha - 1, -alpha,
                                 ply + 1, child,
                                 pseudo, scores, undo, stop, counters, probe, path_hashes,
                                 game_hashes, game_count, tt_key, tt_score, tt_depth, tt_bound,
                                 tt_move_arr, killers, history,
