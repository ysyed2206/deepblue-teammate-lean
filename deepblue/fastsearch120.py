"""S1-search120: fastsearch4 + a transposition table probe/store inside qsearch.
...
# ProbCut (fastsearch120). If a REDUCED-depth search already beats beta by a
# comfortable margin, a full-depth search very likely would too, so the node
# is cut without paying full price. Reduce-then-verify, the category this
# engine accepts. Its 48.3% rejection is void: measured at the 60ms gate where
# the engine reached depth 4, while ProbCut is gated at depth >= 5, so it
# never executed once.
#
# CAVEAT measured before testing: this simplified form searches the whole
# position at reduced depth, where the standard implementation searches only
# CAPTURES and is far cheaper. Node counts went +9.8%, -6.7%, +10.1% on three
# real positions -- a pruning technique that mostly ADDS nodes is not doing
# its job, which is the same signature singular extensions had (+43-55%
# nodes) before failing at 47.5%. Testing anyway because it is built and the
# cost is one run; a capture-only version is the follow-up if this is close.
PROBCUT_MIN_DEPTH = 5
PROBCUT_MARGIN = 200
PROBCUT_REDUCTION = 4
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if (
        ...
        if abs(probcut_beta) < MATE_THRESHOLD:
            probcut_depth = depth - PROBCUT_REDUCTION
            if probcut_depth > 0:
                probcut_score = negamax(
                    bb, occ, mail, st, probcut_depth,
                    probcut_beta - 1, probcut_beta, ply, value,
                    pseudo, scores, undo, stop, counters, probe,
                    path_hashes, game_hashes, game_count,
                    tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                    killers, history, continuation_history, prev_moves,
                    piece_keys, side_key, castle_keys, ep_file_keys,
