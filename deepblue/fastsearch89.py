"""S1-search89: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Node-fraction time management (fastsearch89): how confident the current
# iteration is in its own best move, measured by search effort rather than
# by whether the move's identity happened to repeat.
#
# MIN_DEPTH is 6, not the 8 first tried, because of a measurement: on an idle
# machine this engine reaches depth 12 on the start position at a 2000 ms
# budget, but only depth 10 on a real competition seed and depth 8 on a real
# round-41 middlegame -- and in the late-middlegame squeeze seen in the
# round-41/42 losses, where the clock is down to 0.7-1.0 s/move, it reaches
# only depth 7-9. Gating at 8 would have left this feature inert in exactly
# the phase it is meant to help. 6 keeps it active across essentially every
# real game move while still ignoring the genuinely noisy first few plies.
NODE_EFFORT_MIN_DEPTH = 6
NODE_EFFORT_HIGH = 0.85   # nearly all effort on one move -> confident, stop sooner
NODE_EFFORT_LOW = 0.40    # effort spread thin -> uncertain, keep searching
EFFORT_SCALE_MIN = 0.6    # soft-budget multiplier at HIGH confidence
EFFORT_SCALE_MAX = 1.5    # soft-budget multiplier at LOW confidence
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
                ...
                tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
                killers, history, piece_keys, side_key, castle_keys, ep_file_keys,
                root_moves, root_scores, root_move_nodes, previous_best, root_alpha, root_beta):
    side = st[0]
    path_hashes[0] = value
    ...
    if legal > 0:
        ...
        if best != 0:
            ...
            root_move_nodes[0], root_move_nodes[best] = root_move_nodes[best], root_move_nodes[0]
    return legal
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                completed = depth
                total_iteration_nodes = 0
                for _i in range(legal):
                    total_iteration_nodes += int(self.root_move_nodes[_i])
                if total_iteration_nodes > 0:
                    node_fraction = int(self.root_move_nodes[0]) / total_iteration_nodes
                else:
                    node_fraction = 1.0
                ...
                if depth >= NODE_EFFORT_MIN_DEPTH:
                    if node_fraction >= NODE_EFFORT_HIGH:
                        scale = EFFORT_SCALE_MIN
                    elif node_fraction <= NODE_EFFORT_LOW:
                        scale = EFFORT_SCALE_MAX
                    else:
                        span = NODE_EFFORT_HIGH - NODE_EFFORT_LOW
                        weight = (node_fraction - NODE_EFFORT_LOW) / span
                        scale = EFFORT_SCALE_MAX + weight * (EFFORT_SCALE_MIN - EFFORT_SCALE_MAX)
                    effective_soft_ms = min(hard_ms * 0.9, soft_ms * scale)
                if elapsed >= effective_soft_ms:
                    break
                ...
                if (elapsed + iteration_ms * growth > effective_soft_ms
                        and elapsed > effective_soft_ms * 0.5):
                    break
