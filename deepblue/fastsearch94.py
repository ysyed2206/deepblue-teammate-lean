"""S1-search94: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Transposition table size (fastsearch94). Was 20 bits = 1,048,576 entries
# (~20MB). Measured node counts make that indefensible: a single 2-3 second
# move searches 1.4-1.9 MILLION nodes, so one move already overflows the
# whole table. The table is deliberately NOT cleared between moves (search()
# resets only counters and killers), i.e. it is designed to accumulate
# knowledge across the game -- but at this size every move destroys the
# previous move's work, and the damage compounds as the game lengthens.
# That is a direct, mechanical explanation for late-game depth being worse
# than opening depth, independent of how the clock is allocated.
# 22 bits = 4,194,304 entries (~80MB), which is comfortably within any
# plausible platform memory limit and roughly 2-3x the per-move node count,
# so a move's own search no longer evicts itself.
TT_BITS = 22
TT_SIZE = 1 << TT_BITS
