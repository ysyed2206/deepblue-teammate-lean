"""S1-search162: fastsearch4 + a transposition table probe/store inside qsearch.
...
fastsearch187 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.

The lineage since 135 is: 140 added the queen king-attack weight (10 -> 202)
and nothing else; 150 added history-modulated LMR, multicut and a clock floor
(+11 Elo, 520 games); 162 added singular extensions (+32.5 +/-29, 300 fixed
games). Only ONE of those is a change to the evaluation, and only that one has
evidence against it from real games:

    build          tournament games   score
    fastsearch135        6            75.0%
    fastsearch140        3            33.3%     <- 135 + Q=202, no search change
    fastsearch150        4            12.5%
    fastsearch162        2            50.0%

Those samples are far too small to be conclusive on their own. What makes 140
the suspect is that it isolates the variable: it is 135 with one constant
changed. Independently, fitting the four king-attack weights to 120k quiet
positions carrying deep Stockfish scores ranks Q=202 as the WORST value
anywhere in 10..202, while 135's Q=10 sits within noise of the fitted optimum
(validation MSE 0.031900 against 0.031850 at Q=100, versus 0.032537 at 202).

So this build reverts the evaluation to exactly 135's -- the same danger
function, the same 81/81/121/10 weights -- and keeps the two search changes,
which are the best-measured results this project has.

WHAT THIS BUILD IS BETTING. Self-play says the search additions are worth
about +43 Elo; the tournament record says the lineage got worse. Both cannot
be right. This bets that the evaluation change was the problem and the search
changes are real. If the tournament disagrees, the next thing to strip is the
search, not the eval.

fastsearch171 remains the alternative: same search, but all four weights moved
to their fitted values. The evidence only implicates the queen, so 171 changes
more than the data demands.

fastsearch187 (2026-09-09) = fastsearch180 + BUDGET-NEUTRAL stability timing.

Round 82 was lost to 19...b4. At fixed depth no build of this engine ever
plays it: depth 10-11 prefers Reb8, 12-13 Rf8, 14+ Qb7, all keeping us at
+22 to +80. b4 only appears in a narrow shallow window, and the game reached
roughly that window in its 3.14s.

The signal was available at the time. The root move changed at nearly every
iteration -- bxa4, Reb8, Rf8, Qb7 -- which is what an unresolved position
looks like from inside the search. A position whose best move has been the
same for several plies is finished; one that changes every ply is not.

WHY BUDGET-NEUTRAL. The obvious version of this ("think longer when unsure")
is wrong here, and measurement is what said so. Across 94 real games we spend
3.15s per move in moves 1-15 against opponents' 3.73s, we end games with 31.4s
against their 32.4s, and we drop under 10 seconds in 23 games to their 13.
There is no spare time to spend. So this takes time FROM stable positions and
gives it to unstable ones, leaving the average where it is.

    root move changed on the last iteration    x1.60
    unchanged for 4+ iterations                x0.90
    otherwise                                  x1.00

CALIBRATION. The first attempt (1.45 / 0.72) was measured over 120 positions
at a 2500ms budget and came out 11.4% CHEAPER than the build it modifies,
losing 0.22 ply on average -- most positions are stable, so the discount
dominated and the change was a time cut wearing a reallocation's clothes.
These values were set to bring the total back to neutral while widening the
gap between the two cases.

The hard limit is untouched, so this cannot cause a flag: elapsed >= hard_ms
still ends the search, and the soft scaling is clamped to the hard limit.

fastsearch187 (2026-09-09) = fastsearch180 + stability timing + the ply-1
repetition rule, deliberately BUNDLED.

Each half is too small to measure alone. The stability timing (fastsearch182)
ran 160 games against 180 and landed at +4.3 +/-41 -- which is not "no effect",
it is "no information about anything under about 40 Elo". The repetition rule
(fastsearch183) is smaller still: it changed one move in eleven in the one game
that motivated it.

Bundling them is Yusuf's instruction and the evidence supports it: this harness
resolves roughly +/-65 Elo at 60 games and +/-30 at 300, so a change worth +10
reads ~51% and gets discarded. Singular extensions was thrown away that way at
60 games and later measured +32.5 +/-29 over 300. Two changes worth +10 each
are +20 together, which is detectable where neither is alone.

They are independent -- one decides how long to search, the other what a
repeated position scores -- so a bad interaction is unlikely, which is the
condition under which this project allows bundling at all.

Motivation for each, from real games:
  timing      Round 82's 19...b4 and round 84's 14...gxh6 are BOTH moves the
              engine prefers at exactly one depth and abandons at the next,
              and in both the root move was still changing every iteration.
              That instability is visible while searching.
  repetition  Round 79 drew a +195 position by repeating; at the second
              occurrence nothing in the old rule fired.

fastsearch187 (2026-09-10) = fastsearch185 + TRANSPOSITION TABLE AGEING.

The replacement rule in 185 is `tt_depth[index] <= depth or tt_key[index] !=
value` -- pure depth-preferred, with no notion of WHEN an entry was written. An
entry stored at depth 14 on move 5 can never be evicted by a shallower one, so
it survives the whole game and keeps being probed and trusted long after the
position it summarised has gone.

Stockfish has exactly the field we lack: a generation counter, so entries from
older searches are preferentially replaced even when they are deeper. That is
how a strong engine keeps its table across moves without the table going stale
-- which is Yusuf's point: history is not the problem, our handling of it is.

fastsearch154 implemented the proper per-entry generation and measured -7.9
Elo. That number resolves nothing: this harness is +/-30 at 300 games and 154's
sample was far smaller. It was discarded on no evidence.

WHY DEPTH DECAY RATHER THAN A GENERATION FIELD. A real generation counter has to
be threaded through negamax, quiescence and search_root, and argument-threading
misses have silently broken three candidates in this project already (hence
tools/check_calls.py). Decrementing every stored depth by one at the start of
each move achieves the same two effects with a single line and no signature
changes:

  * an old entry's depth falls each move, so it loses replacement priority and
    is evicted once something shallower but current arrives;
  * it also stops satisfying `tt_depth >= depth` for cutoffs, so a stale entry
    is re-searched rather than trusted.

Entries from the immediately-preceding search keep almost all their value --
they lose one ply -- while an entry ten moves old has decayed by ten and is
effectively gone. Cost is one numpy pass over 4M int16 per move, under 10ms
against a ~2000ms budget.

Nothing about correctness changes: a decayed entry is still a valid score for
its position, it is simply trusted for less.
"""
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    ...
    extra += king_attack_danger_squares_white_relative(bb, phase, _TOTAL_PHASE)
    extra += mobility_white_relative(bb)
    # Bishop pair: two bishops cover both square colours and complement
    # each other as the board opens. The base evaluation prices every
    # bishop identically, so trading one off currently costs nothing.
    extra += bishop_pair_white_relative(bb)
    # Doubled and isolated pawns. Piece-square tables score a pawn purely by
    # where it stands, so two pawns stacked on one file score as well as two
    # abreast, and a pawn with no neighbours as well as one in a chain. Both
    # are structural weaknesses the search cannot find tactically -- they cost
    # material far beyond the horizon, which is the shape of losing gradually
    # rather than to a blunder.
    extra += pawn_structure_white_relative(bb)
    # BUNDLE (fastsearch114). Rook placement measured 51.9% over 80 games and
    # knight outposts 51.2%; both were individually discarded as neutral. That
    # verdict is a statement about MEASUREMENT, not about the features: at
    # 80-120 games this harness resolves roughly +/-30 Elo, so a change truly
    # worth +10 is invisible on its own and reads ~51%. Three such changes
    # together would be +30, which is detectable.
    #
    # Precedent in this engine: fastsearch57 deliberately bundled two
    # individually-tested, individually-inconclusive techniques and measured
    # 53.1% over 480 games. The rule this breaks (one idea per candidate)
    # exists to stop a bad component hiding behind a good average; that risk is
    # much lower here because each component was already measured as not
    # harmful on its own.
    extra += rook_files_white_relative(bb)
    extra += knight_outposts_white_relative(bb)
    if st[0] != _WHITE:
        extra = -extra
    return score + extra
...
# 22 bits (4M entries, ~80MB against a 2GB limit) measured 50.8% alone.
TT_BITS = 22
TT_SIZE = 1 << TT_BITS
...
# Contempt (fastsearch116). DRAW_SCORE is 0, so a repetition scores exactly
# equal to an unclear position and the engine takes a draw whenever its own
# evaluation is even slightly negative. Round 53 is the concrete cost: a
# threefold repetition at MOVE 15 as White, from a position the engine rated
# -15. By its own arithmetic a draw (0) beat -15, so it repeated on purpose.
# In a Swiss where the team must climb from 77th, an early draw as White is
# nearly as bad as a loss.
#
# Applied ONLY to repetition. Stalemate, insufficient material and the
# fifty-move rule are genuinely drawn and must still score exactly 0 -- a
# first attempt replaced all thirteen DRAW_SCORE sites and failed four
# regression positions asserting that a terminal draw scores 0.
CONTEMPT = np.int64(20)
...
# Razoring (fastsearch118). The mirror of RFP: RFP asks whether the static
# eval is far enough ABOVE beta to assume a fail-high; razoring asks whether
# it is far enough BELOW alpha that the node is hopeless. Crucially it does
# NOT trust the static eval alone -- quiescence VERIFIES before it cuts.
# This engine rejects discard techniques (LMP scored 5%) and accepts
# reduce-then-verify ones (null move, LMR, SEE pruning); razoring is the
# second kind. Its recorded rejection came from the 60ms gate at depth 4,
# where a depth<=3 condition barely fires, so that verdict is void.
# Clock: raise the predictive-stop floor. Round 72 spent 3.57s/move in the
# opening and 1.62s/move in the endgame where it bled 700cp, using only 56%
# of an already-allocated budget. An aborted iteration is discarded, so the
# downside is wasted work, never a lost move.
PREDICTION_MIN_FRACTION = 0.80
UNSTABLE_SOFT_SCALE = 1.60
STABLE_SOFT_SCALE = 0.90
STABLE_PLIES = 4

RAZOR_MAX_DEPTH = 3
RAZOR_MARGIN_BASE = 180
RAZOR_MARGIN_PER_DEPTH = 120
...
# Late move pruning. Counts are deliberately generous -- every measured win in
# this engine has come from conservative gating.
LMP_MAX_DEPTH = 3
LMP_COUNT = np.array([0, 6, 12, 20], dtype=np.int64)

# Delta pruning in quiescence. A capture is skipped when the stand-pat score
# plus the captured piece plus this margin still cannot reach alpha -- i.e.
# even winning that piece for free would not matter. The margin covers the
# positional swing a capture can produce beyond raw material.
DELTA_MARGIN = np.int64(200)

# SEE pruning in the MAIN search: at shallow non-PV nodes, a capture that
# loses material by force is not worth a subtree.
SEE_PRUNE_MAX_DEPTH = 3

# Internal iterative reduction: no TT move at a deep enough non-PV node means
# the node has not been worth storing before, so search it one ply shallower.
IIR_MIN_DEPTH = 4

# Singular extensions. Conservative gates: deep nodes only, and the stored
# entry must be a real lower bound from a search nearly as deep as this one.
SINGULAR_MIN_DEPTH = 8
SINGULAR_TT_DEPTH_SLACK = 3
SINGULAR_MARGIN = 2

# History-modulated LMR: reduce less for moves the history likes. Free -- the
# score is already computed for ordering and then discarded.
LMR_HISTORY_DIV = np.int64(8192)
LMR_HISTORY_MAX_ADJ = np.int64(2)

# Multi-cut: if several best-ordered moves all fail high at reduced depth,
# treat the node as a proven fail-high. Reduce-then-verify family.
MULTICUT_MIN_DEPTH = 6
MULTICUT_REDUCTION = 4
MULTICUT_MOVES = 6
MULTICUT_CUTOFFS = 3

# Correction history. Entries are a gravity-updated running estimate of
# (search score - static eval) for positions sharing a pawn structure.
CORR_BITS = 14
CORR_SIZE = 1 << CORR_BITS
CORR_MASK = np.uint64(CORR_SIZE - 1)
CORR_LIMIT = np.int64(1024)      # entry saturation
CORR_QUADRATIC_DIV = np.int64(11175)   # entry^2 -> centipawns (Stockfish's divisor)
CORR_MAX_SHIFT = np.int64(128)   # never move the eval more than this
...
@njit(cache=True, nogil=True)
def pawn_structure_key(bb, piece_keys):
    """Zobrist hash over pawns only -- the index for correction history.

    Pawn structure is the standard index because it changes slowly and
    characterises the kind of position, so the correction learned in one
    node transfers to its neighbours. Recomputed from scratch rather than
    maintained incrementally: measured at roughly 30ns against a ~2400ns
    node, and an incremental version is another thing to get wrong.
    """
    key = np.uint64(0)
    bits = bb[0]
    while bits:
        ...
        key ^= piece_keys[0, square]
    bits = bb[6]
    while bits:
        square = lsb(bits)
        bits &= bits - np.uint64(1)
        key ^= piece_keys[6, square]
    return key


@njit(cache=True, nogil=True)
def corrected_eval(raw, corr_hist, side, pawn_key):
    """Static eval shifted by what this pawn structure has historically cost."""
    entry = corr_hist[side, pawn_key & CORR_MASK]
    # QUADRATIC in the entry, as Stockfish does (v += cv * abs(cv) / 11175),
    # not linear. The shape is the point: a correction the search has only
    # weakly confirmed barely moves the evaluation, while one confirmed over
    # and over moves it a lot. fastsearch132 used entry//8, which at entry=256
    # applies 32cp where the reference form applies 6cp -- five times too much
    # for a correction that has barely been established.
    magnitude = entry if entry >= 0 else -entry
    shift = (entry * magnitude) // CORR_QUADRATIC_DIV
    if shift > CORR_MAX_SHIFT:
        shift = CORR_MAX_SHIFT
    elif shift < -CORR_MAX_SHIFT:
        shift = -CORR_MAX_SHIFT
    return raw + shift


@njit(cache=True, nogil=True)
def update_corr_hist(corr_hist, side, pawn_key, diff, depth):
    """Gravity update, matching the rule the main history already uses."""
    bonus = diff * depth // 8
    quarter = CORR_LIMIT // 4
    if bonus > quarter:
        bonus = quarter
    elif bonus < -quarter:
        bonus = -quarter
    index = pawn_key & CORR_MASK
    entry = corr_hist[side, index]
    magnitude = bonus if bonus >= 0 else -bonus
    corr_hist[side, index] = entry + bonus - (entry * magnitude) // CORR_LIMIT
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    for index in range(count):
        ...
        if (move >> np.uint32(20)) & np.uint32(15) == np.uint32(NO_PIECE):
            # Delta pruning, before SEE because it is far cheaper: if even
            # winning the captured piece outright leaves us short of alpha,
            # the subtree cannot matter. Promotions are excluded by the guard
            # above; en-passant and quiet checks (captured == NO_PIECE) are
            # left alone because there is no captured value to reason about.
            delta_captured = (move >> np.uint32(16)) & np.uint32(15)
            if delta_captured < np.uint32(12):
                if stand_pat + SEE_VALUE[delta_captured] + DELTA_MARGIN <= alpha:
                    continue
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    if ply > 0 and not suppress_repetition:
        ...
        if prior_path >= 1:
            return -CONTEMPT
        # ONE prior occurrence, but ONLY at the move we are about to play.
        # Applying this at every ply (fastsearch173) collapses a shuffling
        # endgame's whole tree to +/-CONTEMPT and the engine can no longer
        # rank its options; restricted to ply 1 it halved the repeating
        # choices in round 79 (2 of 11 -> 1 of 11) with scores unaffected.
        if ply == 1 and prior_total >= 1:
            return -CONTEMPT
    if insufficient_material(bb, occ):
        return DRAW_SCORE
    ...
    counters[TTPROBE] += 1
    tt_hit = False
    tt_stored = np.int64(0)
    tt_bound_val = np.int64(0)
    tt_stored_depth = np.int64(-1)
    if tt_usable and tt_key[index] == value:
        counters[TTHIT] += 1
        tt_move = tt_move_arr[index]
        tt_hit = True
        tt_stored = np.int64(score_from_tt(tt_score[index], ply))
        tt_bound_val = np.int64(tt_bound[index])
        tt_stored_depth = np.int64(tt_depth[index])
        # A TT cutoff while a move is excluded would return the score of the
        # position INCLUDING that move, which is the thing being tested.
        if tt_depth[index] >= depth and ply > 0 and excluded[ply] == np.uint32(0):
    ...
    # One static evaluation per node, corrected by what this pawn structure
    # has historically been worth, and shared by reverse futility, razoring,
    # null move and futility below. Those four each called evaluate() on this
    # same unchanged position before; more importantly, correction history
    # needs a single value to correct and a single place to learn from.
    node_pawn_key = np.uint64(0)
    node_eval = np.int64(0)
    have_eval = False
    if not checked and not is_pv:
        counters[EVAL] += 1
        ...
        have_eval = True
    # Internal iterative reduction. A node with no transposition-table move has
    # not previously been worth storing, which is evidence it is not critical.
    # Classical IID would run a reduced search here purely to obtain a move;
    # that is the same trade ProbCut (49.4%) and singular extensions (47.5%)
    # both lost in this engine, so this takes the cheap form instead.
    if (
        not is_pv
        and not checked
        and depth >= IIR_MIN_DEPTH
        and tt_move == np.uint32(0)
    ):
        depth -= 1
    ...
    if (
        ...
        razor_eval = node_eval
        if razor_eval + RAZOR_MARGIN_BASE + RAZOR_MARGIN_PER_DEPTH * depth <= alpha:
    ...
    # Singular extension test. Search this position again with the TT move
    # excluded; if everything else fails below a window just under the TT
    # score, that move stands alone and its line earns an extra ply.
    ...
    if (
        ...
        and tt_hit
        and tt_move != np.uint32(0)
        and excluded[ply] == np.uint32(0)
        and tt_stored_depth >= depth - SINGULAR_TT_DEPTH_SLACK
        and (tt_bound_val == LOWER or tt_bound_val == EXACT)
        and tt_stored > -MATE_THRESHOLD
        and tt_stored < MATE_THRESHOLD
        and not checked
    ):
        singular_beta = tt_stored - np.int64(SINGULAR_MARGIN * depth)
        singular_depth = (depth - 1) // 2
        if singular_depth > 0:
            excluded[ply] = tt_move
            singular_score = negamax(
                ...
                True, False)
            excluded[ply] = np.uint32(0)
            if not stop[0] and singular_score < singular_beta:
    ...
    if (
        ...
        and not checked
        and can_null
        and depth >= MULTICUT_MIN_DEPTH
        and abs(beta) < MATE_THRESHOLD
    ):
        mc_reduced = depth - MULTICUT_REDUCTION
        if mc_reduced > 0:
            mc_cuts = 0
            mc_tried = 0
            for mc_index in range(count):
                if mc_tried >= MULTICUT_MOVES:
                    break
                mc_move = pick_best(pseudo[ply], scores[ply], count, mc_index)
                mc_old_castle = st[1]
    ...
    for order_index in range(count):
        ...
        if (
            ...
            continue
        # SEE pruning in the main search: a capture that loses material by
        # force, at a shallow non-PV node, does not earn a subtree. Quiescence
        # already does this; the main search did not.
        if (
            not is_pv
            and not checked
            ...
            and move != tt_move
            and abs(alpha) < MATE_THRESHOLD
            and not see_ge_zero(bb, occ, mail, st, move)
        ):
            continue
        old_castle = st[1]
        old_ep_file = Z.canonical_ep_file(bb, occ, st)
    ...
    # Learn from this node: the gap between what the static evaluation said and
    # what the search actually found. Skipped for mate scores (not an
    # evaluation error) and when no static eval was taken.
    if (
        have_eval
        and not stop[0]
        and legal > 0
        ...
        update_corr_hist(corr_hist, side, node_pawn_key,
                         np.int64(best) - node_eval, np.int64(depth))
    ...
    if tt_usable and not stop[0] and excluded[ply] == np.uint32(0):
        if best <= original_alpha:
            bound = UPPER
        elif best >= beta:
            bound = LOWER
...
class FastEngine187:
    ...
    def __init__(self) -> None:
        ...
        # move excluded from the search at each ply, for singular testing
        self.excluded = np.zeros(MAX_PLY + 2, dtype=np.uint32)
        # correction history: [side][pawn structure hash]
        self.corr_hist = np.zeros((2, CORR_SIZE), dtype=np.int64)
    ...
    def search(self, position, soft_ms: float, hard_ms: float, max_depth: int = 64):
        ...
        self.excluded[:] = 0
        stable_for = 0
        # TT ageing by depth decay -- see the module docstring. Entries written
        # for earlier positions lose a ply of authority per move, so they fall
        # out of both the replacement policy and the cutoff test instead of
        # outliving the game.
        np.subtract(self.tt_depth, 1, out=self.tt_depth,
                    where=self.tt_depth > 0)
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                ...
                completed = depth
                # Stability of the ROOT move, not of the score: a score can
                # drift while the move stays obvious, and it is the move we
                # are being asked for.
                if completed > 1 and best_move != previous_best:
                    stable_for = 0
                else:
                    stable_for += 1
                ...
                # Budget-neutral reallocation (see the module docstring).
                if stable_for == 0:
                    soft_now = soft_ms * UNSTABLE_SOFT_SCALE
                elif stable_for >= STABLE_PLIES:
                    soft_now = soft_ms * STABLE_SOFT_SCALE
                else:
                    soft_now = soft_ms
                if soft_now > hard_ms:
                    soft_now = hard_ms
                ...
                if (elapsed + iteration_ms * growth > soft_now
                        and elapsed > soft_now * PREDICTION_MIN_FRACTION):
                    break
