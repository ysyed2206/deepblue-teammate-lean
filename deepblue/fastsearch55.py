"""S1-search55: fastsearch4 + a transposition table probe/store inside qsearch.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # Mate distance pruning (chessprogramming.org/Mate_Distance_Pruning).
    # No line from this node can be better than "mate in one more ply" or
    # worse than "already mated here" -- if alpha/beta have already
    # converged past what any such score could improve on, there is nothing
    # left to search for. Complements the mate-score early-stop fix: that
    # fix made the iterative-deepening LOOP verify a mate score properly;
    # this makes the search TREE itself stop wasting nodes once a mate
    # bound is already unbeatable.
