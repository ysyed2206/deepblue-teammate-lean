"""S1-search139: fastsearch4 + a transposition table probe/store inside qsearch.
...
# History-modulated late move reductions. A history score is already computed
# for every quiet move in order to ORDER it, then discarded when deciding how
# far to REDUCE -- so a move the history strongly likes is reduced exactly as
# hard as one it has never seen succeed. Standard in modern engines (Stockfish
# derives its reduction adjustment from a combined stat score over main and
# continuation histories).
#
# HISTORY_MAX is 16384, so 8192 gives about +/-2 plies from the main table
# before continuation history is added; clamped to +/-2 so one saturated entry
# cannot cancel the reduction outright or double it.
