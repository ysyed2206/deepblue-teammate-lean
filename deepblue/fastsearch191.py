...
fastsearch191 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.
...
fastsearch191 (2026-09-09) = fastsearch180 + BUDGET-NEUTRAL stability timing.
...
fastsearch191 (2026-09-09) = fastsearch180 + stability timing + the ply-1
...
fastsearch191 (2026-09-10) = fastsearch185 + LOSING CAPTURES ORDERED LAST.

185 scores every capture with ORDER_CAPTURE (1<<21), which sits above
ORDER_KILLER_1 (1<<20) and far above any history score. So QxP defended by a
pawn -- static exchange -800 -- is searched BEFORE the killer moves and before
every good quiet. Only the victim/attacker values are consulted; whether the
capture actually wins material is never asked.

Every strong engine splits captures on static exchange evaluation: winning and
equal captures first, then killers, then quiets by history, then LOSING captures
LAST. Ordering is where this engine's measured deficit is -- instrumentation
earlier in this project put the first-move cutoff rate at 85.1% overall, 75.4%
in the Italian and 77.9% in the middlegame, against a ~90% benchmark -- and every
confirmed Elo gain in this project has come from search rather than evaluation.

see_ge_zero was already imported and used for pruning; it just was not consulted
for ordering. This threads the board state into order_moves so it can be.

    ORDER_TT          1<<24
    ORDER_PROMOTION   1<<22
    ORDER_CAPTURE     1<<21    <- good/equal captures only, now
    ORDER_KILLER_1    1<<20
    ORDER_KILLER_2    (1<<20)-1
    history           0 .. (1<<19)-1
    ORDER_BAD_CAPTURE negative  <- new: below every quiet

THREADING RISK, stated because it has bitten this project three times: this
changes order_moves' signature, and there are exactly two call sites (negamax
and search_root). tools/check_calls.py verifies every call matches the
definition and is run below -- an assert on a token count would NOT catch a
missed site, which is how fastsearch132, 154 and 162 each shipped broken.
"""
...
# Below every quiet: a capture that loses material should not be tried
# before a move the history tables like. See the module docstring.
ORDER_BAD_CAPTURE = -(1 << 21)
...
def order_moves(moves, count, scores, mail, tt_move, killers, history,
                continuation_history, prev_moves, ply, bb, occ, st):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            # Winning and equal captures keep their place above the killers;
            # losing ones drop below every quiet instead of being searched
            # ahead of moves that are actually good.
            if see_ge_zero(bb, occ, mail, st, move):
                scores[index] = ORDER_CAPTURE + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
            else:
                scores[index] = ORDER_BAD_CAPTURE + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history,
                continuation_history, prev_moves, 0, bb, occ, st)
