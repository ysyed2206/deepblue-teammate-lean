"""S1-search56: fastsearch4 + a transposition table probe/store inside qsearch.
...
# Check extension: search one ply deeper than nominal when the current node
# is in check, since a forced check-evasion sequence deserves the same
# scrutiny as a capture chain, not the horizon truncation a fixed-depth cut
# would otherwise give it. Applied inside negamax's own move loop only
# (search_root's few calls are left untouched to keep this isolated).
CHECK_EXTENSION = 1
