"""S1-search16: fastsearch4 + TT score cutoffs restricted to non-PV nodes.
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
    ...
    # A full-window/PV node may still use the stored move for ordering, but a
    # bound from another path must not truncate the principal variation.
    ...
    if tt_usable and tt_key[index] == value:
        ...
        if not is_pv and tt_depth[index] >= depth and ply > 0:
