"""S1-search128: fastsearch4 + a transposition table probe/store inside qsearch.
...
# How much of the soft budget must be spent before the predictive stop is
# allowed to end the search.
#
# Measured across 72 real games (2026-09-07): the engine leaves 37.9s of its
# 120s unspent, averaged over wins, draws and losses alike, and spends 2.87s
# per move in the first third of its moves against 1.50s in the last third.
# The cause is here rather than in time_manager: late in the game the soft
# budget is ~2.9s while actual spend is ~1.5s, i.e. 52% -- the predictive stop
# fires at the earliest instant this floor permits, on essentially every move.
# The prediction has stopped discriminating between positions and become a
# flat halving of the budget.
#
# Raising the floor spends more of the allocation. The cost is bounded and
# known: an aborted iteration is discarded (see the `stop[0]` break in the
# deepening loop), so the downside is wasted work, never a lost move -- the
# hard timer still ends the search and the last completed iteration's move
# stands. The upside is a full extra ply whenever the gamble lands, and depth
# is what this engine is short of: in round 58 it plays the losing g4 at
# depth 11 and the correct Nf3 at depth 13.
