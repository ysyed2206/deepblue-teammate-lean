...
fastsearch192 (2026-09-09) = fastsearch162's SEARCH with fastsearch135's EVAL.
...
fastsearch192 (2026-09-09) = fastsearch180 + BUDGET-NEUTRAL stability timing.
...
fastsearch192 (2026-09-09) = fastsearch180 + stability timing + the ply-1
...
fastsearch192 (2026-09-10) = fastsearch185 with a LINEAR history bonus.

From DONOR_NEXT_IDEAS.md, "History decomposition (priority #2)", whose own
smallest-first list is: 1. gravity only (185 already has it), 2. BONUS SHAPE
ONLY. This is step 2.

185 uses min(depth*depth, 1536) -- the depth-squared family. The donor survey
found two live families and a clear majority:

    linear-capped (the modern consensus)
      Stockfish    min(133*d - 81, 1487)
      Reckless     min(184*d, 1742) - 72 - 42*cut_node
      Coda         clamp(0, 1653, 245*d - 18)
      Viridithas   min(mul*d + offset, max)

    depth-squared
      Ethereal     16*d*d + 128*max(d-1, 0)
      Pawnstar     min(d*d, 400)

Four donors to two. This adopts Stockfish's exact shape, min(133*d - 81, 1487),
clamped at zero so depth 0 cannot produce a negative bonus.

Why the shape matters rather than just the scale: depth-squared makes a deep
cutoff worth quadratically more than a shallow one, so a single deep node can
dominate the table. At depth 12 our formula gives 144 against the linear
1515 -- but at depth 3 it gives 9 against 318. The squared form is far too
flat where most nodes actually are, and only catches up at depths that are
rare. HISTORY_MAX stays 16384, which the donor survey confirms is the
majority constant and was NOT the error in an earlier failed attempt here.

The malus uses the same value, as it does in 185 -- keeping this candidate to
ONE change. Giving malus its own independently-tunable slope (Coda, Reckless
and Stockfish all do) is the next separate candidate, along with Reckless's
division of malus by the count of quiets already punished at that node, which
stops one wide node mass-poisoning the table. Neither is bundled here.
"""
...
HISTORY_MAX = 16384
# Stockfish's min(133*d - 81, 1487). Replaces min(d*d, 1536).
HISTORY_BONUS_MAX = 1487
HISTORY_BONUS_SLOPE = 133
    ...
    for order_index in range(count):
        ...
        if alpha >= beta:
            ...
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                ...
                piece = (move >> np.uint32(12)) & np.uint32(15)
                to_square = (move >> np.uint32(6)) & np.uint32(63)
                # Linear-capped, Stockfish's shape. See the module docstring.
                bonus = HISTORY_BONUS_SLOPE * depth - HISTORY_BONUS_OFFSET
