"""S1-search22: fastsearch18 + SEE good/bad capture ordering in qsearch
("Design A").

Priority-list item #3, Design A half -- independent of fastsearch21's
Design B (bad-noisy skip); the two are deliberately not stacked, per Agent
C's design sketch and this project's one-idea-per-candidate rule.

order_qmoves now takes the position (bb, occ, mail, st) and splits captures
into two qsearch-only score bands: ORDER_CAPTURE_GOOD (== the existing
ORDER_CAPTURE value, unchanged) for SEE>=0 captures, ORDER_CAPTURE_BAD (a
new, lower band with verified headroom -- see the module-level asserts) for
SEE<0 captures. Promotions and capture-promotions are excluded from the SEE
call (see.py's own docstring: not validated for this) and always score via
the unchanged ORDER_CAPTURE_GOOD/ORDER_PROMOTION path. SEE is only called
when count > 1 (a single tactical candidate has nothing to be reordered
against). Ordering only -- no move is skipped, so this candidate cannot
change which moves are searched, only the order, and is expected to be
node-count-neutral-to-positive with identical or move-tie-only-different
best answers at fixed depth versus fastsearch18.
...
# Design A (SEE-based qsearch ordering) qsearch-only bands. ORDER_CAPTURE_GOOD
# reuses ORDER_CAPTURE unchanged; ORDER_CAPTURE_BAD sits strictly below it
# and strictly above 0 (an unscored quiet check-evasion), with headroom to
# spare for the MVV-LVA tie-break term (PIECE_VALUE max 20, so the term is
# bounded well within +/-160). Main search's order_moves/killer bands are
# untouched.
ORDER_CAPTURE_GOOD = ORDER_CAPTURE
ORDER_CAPTURE_BAD = 1 << 19
_MVV_LVA_BOUND = 20 * 16
assert ORDER_CAPTURE_BAD + _MVV_LVA_BOUND < ORDER_CAPTURE_GOOD
assert ORDER_CAPTURE_GOOD + _MVV_LVA_BOUND < ORDER_PROMOTION
...
@njit(cache=True, nogil=True)
def order_qmoves(bb, occ, mail, st, moves, count, scores):
    """Qsearch ordering, SEE-based good/bad capture split ("Design A").

    ORDER_CAPTURE_GOOD reuses fastsearch18's own ORDER_CAPTURE value
    unchanged, so a SEE>=0 capture ranks exactly where today's captures
    already rank relative to ORDER_PROMOTION. ORDER_CAPTURE_BAD sits well
    below that floor (headroom checked at module load, see the assert
    near the constant) and strictly above the 0 a quiet check-evasion
    scores, so a bad capture still outranks unscored quiets but never a
    good one or a promotion.

    SEE is only called when there is more than one tactical candidate (a
    single candidate has nothing to be reordered against) and never on a
    promotion or capture-promotion (deepblue/see.py's own docstring: not
    validated for pruning/ordering promotions in this version).
    """
    for index in range(count):
        move = moves[index]
        piece = (move >> np.uint32(12)) & np.uint32(15)
        ...
        if captured != np.uint32(NO_PIECE):
            if promotion == np.uint32(NO_PIECE) and count > 1:
                if see_value(bb, occ, mail, st, move) >= 0:
                    score += ORDER_CAPTURE_GOOD + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
                else:
                    score += ORDER_CAPTURE_BAD + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
            else:
                score += ORDER_CAPTURE_GOOD + PIECE_VALUE[captured] * 16 - PIECE_VALUE[piece]
...
def quiescence(bb, occ, mail, st, alpha, beta, ply, value, pseudo, scores, undo, stop, counters,
    ...
    if checked:
        ...
        order_qmoves(bb, occ, mail, st, pseudo[ply], count, scores[ply])
        best = -INFINITY
