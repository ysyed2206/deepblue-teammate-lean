"""S1-search108: fastsearch4 + a transposition table probe/store inside qsearch.
...
HISTORY_MAX = 16384
# Capture history (fastsearch108). MVV-LVA orders captures purely by the
# material on the two squares, so QxP and QxP are identical wherever they
# occur and the engine never learns that a particular capture keeps failing.
# This adds a learned term indexed by (piece, to-square, captured piece),
# updated with the same gravity rule as the quiet history.
#
# Bounded at half HISTORY_MAX deliberately: MVV-LVA spans about 0-14400
# (PIECE_VALUE[queen] * 16), so a full-range history term would swamp the
# material ordering rather than refine it. At 8192 it can reorder captures of
# similar material without ever putting a pawn capture above a queen capture.
CAPTURE_HISTORY_MAX = 1024
...
def order_moves(moves, count, scores, mail, tt_move, killers, history,
                continuation_history, capture_history, prev_moves, ply):
    for index in range(count):
        ...
        if move == tt_move:
            ...
            scores[index] = (ORDER_CAPTURE + PIECE_VALUE[captured] * 16
                             - PIECE_VALUE[piece]
                             + capture_history[piece, to_square, captured])
...
def negamax(bb, occ, mail, st, depth, alpha, beta, ply, value, pseudo, scores, undo, stop,
            ...
            tt_key, tt_score, tt_depth, tt_bound, tt_move_arr,
            killers, history, continuation_history, capture_history, prev_moves,
            piece_keys, side_key, castle_keys, ep_file_keys,
    ...
    for order_index in range(count):
        ...
        if legal == 1:
            score = -negamax(bb, occ, mail, st, child_depth, -beta, -alpha, ply + 1, child,
                             ...
                             tt_move_arr, killers, history, continuation_history, capture_history, prev_moves,
                             piece_keys, side_key, castle_keys, ep_file_keys,
                             False, True)
        else:
            reduction = 0
        ...
        if alpha >= beta:
            ...
            if captured != np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
                cap_piece = (move >> np.uint32(12)) & np.uint32(15)
                cap_to = (move >> np.uint32(6)) & np.uint32(63)
                cap_bonus = depth * depth
                if cap_bonus > HISTORY_BONUS_MAX:
                    cap_bonus = HISTORY_BONUS_MAX
                if cap_piece < np.uint32(12) and captured < np.uint32(12):
                    cap_entry = capture_history[cap_piece, cap_to, captured]
                    capture_history[cap_piece, cap_to, captured] = (
                        cap_entry + cap_bonus
                        - (cap_entry * cap_bonus) // CAPTURE_HISTORY_MAX
                    )
            if captured == np.uint32(NO_PIECE) and promotion == np.uint32(NO_PIECE):
...
def search_root(bb, occ, mail, st, depth, value, pseudo, scores, undo, stop, counters,
    ...
    order_moves(pseudo[0], count, scores[0], mail, previous_best, killers, history,
                continuation_history, capture_history, prev_moves, 0)
...
class FastEngine108:
    ...
    def __init__(self) -> None:
        ...
        # (moving piece, to-square, captured piece)
            ...
            for depth in range(1, max_depth + 1):
                ...
                while True:
                    legal = search_root(
                        ...
                        self.continuation_history, self.capture_history, self.prev_moves,
