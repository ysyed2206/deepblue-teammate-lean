"""S1-search51: fastsearch4 + a transposition table probe/store inside qsearch.
...
def evaluate(bb, st, mg_table, eg_table, phase_table):
    """Base PST evaluation plus mobility.

    Isolated candidate, per this project's one-idea-per-test rule. Knights
    and bishops are weighted heaviest (their whole value is which squares
    they control), rooks and queens least (already long-range). Pseudo-legal
    attack counts, not fully legal -- the field settled on that decades ago
    since legal-only filtering needs a make/unmake per candidate move at
    eval-call frequency, for the same heuristic value.
    """
