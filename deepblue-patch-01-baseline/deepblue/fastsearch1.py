"""S1-search1: search0 plus Zobrist, a transposition table, move ordering,
killers, history and repetition detection. Nothing else.

No pruning, no reductions, no aspiration windows, no SEE. The point is to
convert the measured ~37x raw speed advantage into depth, and to be able to
attribute the change to these features and nothing else.

Node-count convention is unchanged from search0 - one node per negamax entry
and one per quiescence entry - so the two are directly comparable.
"""
